...
hmm_autocorr = function(
  ...
  if (derivatives > 0){
      warning('derivative features not tested with autocorrelation model')
  }

  if (is.null(attr(Xs[[1]], 'lags'))){
    lags = 0
    ind_lag = grep('_lag', colnames(Xs[[1]]))
    if (length(ind_lag) > 0){
      lags = max(as.numeric(gsub('.*(_lag)?', '', colnames(Xs[[1]])[ind_lag])))
      ind_lag0 = (1:M)[-ind_lag]
    }
  } else {
    lags = attr(Xs[[1]], 'lags')
  }
  ...
  ### standardize covariates - working in-place to conserve ry
  ...
  if (control$standardize){
    ...
    if (is.null(attr(Xs, 'missingness_labels'))){
      missingness_labels = lapply(Ts, function(x) rep(1,x)) # all obs have same pattern of missingness (none)
    } else {
      missingness_labels = attr(Xs, 'missingness_labels')
    }

    if (is.null(attr(Xs, 'nonmissing_features'))){
      nonmissing_features = list(1:M) # nothing is missing
    } else {
      nonmissing_features = attr(Xs, 'nonmissing_features')
    }
  ...
  if (par$method %in% c('user-responsibilities', 'random-dirichlet')){
    ...
    par$Sigmas = replicate(K, matrix(0, M, M), simplify=F)
