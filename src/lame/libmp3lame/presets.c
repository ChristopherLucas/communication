...
apply_vbr_preset(lame_global_flags * gfp, int a, int enforce)
{
    ...
    SET_OPTION(quant_comp, (float)set->quant_comp, -1);
    SET_OPTION(quant_comp_short, (float)set->quant_comp_s, -1);
    if (set->expY) {
...
apply_abr_preset(lame_global_flags * gfp, int preset, int enforce)
{
    ...
    SET_OPTION(quant_comp, (float)abr_switch_map[r].quant_comp, -1);
    SET_OPTION(quant_comp_short, (float)abr_switch_map[r].quant_comp_s, -1);
