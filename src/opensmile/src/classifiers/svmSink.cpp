...
SMILECOMPONENT_REGCOMP(cSvmSink)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ct->setField("model","The name of the model file.",(const char *)nullptr);
...
cSvmSink::cSvmSink(const char *_name) :
  cDataSink(_name), modelObj(nullptr),
...
{
  int i;
  if (c==nullptr) { SMILE_IERR(1,"Class name cannot be nullptr in getClassIndex()."); return 0; }
  for (i=0; i<nClasses; i++) {
...
int smileSvmModel::parseLogisticModel(struct smileBinarySvmModel *_s, char *inp)
{
  ...
  if (x != nullptr) {
    ...
    *x = 0;
    ...
    //if (eptr != nullptr && eptr != x+1) { SMILE_IERR(1,"Parse error while parsing logistic model of binary svm model (coeff1), token '%s'",x+1); }
    _s->logit_coeff1 = a;
    eptr = nullptr;
    a = strtod(inp,&eptr);
    //if (eptr != nullptr && eptr != x+1) { SMILE_IERR(1,"Parse error while parsing logistic model of binary svm model (coeff0), token '%s'",inp); }
...
int smileSvmModel::load()
{
  ...
  if (origline != nullptr) free(origline);
...
int smileSvmModel::evaluate(FLOAT_DMEM *v, long N, const char ** winningClass, FLOAT_DMEM ** probs, FLOAT_DMEM *conf, int winningClassMethod, int usePairwiseCoupling)
{
  ...
  if (winningClassMethod == SVMSINK_WINNING_CLASS_VOTE || probs == nullptr || *probs == nullptr) {
	  max = result[0];
	  for (i=1; i<nClasses; i++) {
  ...
  if (showStatsDebug) {
	  ...
	  for (i=0; i<nClasses; i++) {
		  if (probs != nullptr && winningClassMethod == SVMSINK_WINNING_CLASS_PROB) { 
			  Rprintf("SVM result with probs, class %i/%i : probsum=%f , dsum=%f , prob=%f\n",i+1,nClasses,probsum[i],dsum[i],(*probs)[i]);
		  } else if (probs != nullptr && winningClassMethod == SVMSINK_WINNING_CLASS_VOTE) {
			  Rprintf("SVM result, class %i/%i : nVotes=%i , dsum=%f , prob=%f\n",i+1,nClasses,result[i],dsum[i],(*probs)[i]);
		  } else {
			  Rprintf("SVM result, class %i/%i : nVotes=%i , dsum=%f\n",i+1,nClasses,result[i],dsum[i]);
		  }
	  }
  }

  SMILE_IMSG(4,"\n  Winning class: # %i (%s) (dsum=%f , #votes = %i).",maxC,classnames[maxC],dsum[maxC],max);
  if (conf != nullptr) { *conf = (FLOAT_DMEM)fabs(dsum[maxC]); }
...
smileSvmModel::~smileSvmModel()
{
  ...
  if (classnames != nullptr) {
    if (classnames[0] != nullptr) free(classnames[0]);
    free(classnames);
  }
  if (binSvm != nullptr) {
    for (i=0; i<nPairs; i++) {
      if (binSvm[i].c1 != nullptr) free(binSvm[i].c1);
      if (binSvm[i].c2 != nullptr) free(binSvm[i].c2);
      if (binSvm[i].SV != nullptr) free(binSvm[i].SV);
    }
    free(binSvm);
  }
  if (attributeNames != nullptr) {
    for (i=0; i<vectorDim; i++) {
      if (attributeNames[i] != nullptr) free(attributeNames[i]);
    }
    free(attributeNames);
  }
  if (ftSelMap != nullptr)
...
int cSvmSink::myTick(long long t)
{
  ...
  if (sendResult) {
    ...
    msg.custData2    = (void *) ( /*models[modelIdx].modelResultName*/ nullptr ); // dangerous conversion....!
...
cSvmSink::~cSvmSink()
{
  if (modelObj != nullptr) delete modelObj;
}
