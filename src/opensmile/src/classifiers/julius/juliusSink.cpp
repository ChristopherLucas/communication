...
SMILECOMPONENT_REGCOMP(cJuliusSink)
{
  ...
  SMILECOMPONENT_INHERIT_CONFIGTYPE("cDataSink")
    ...
    ct->setField("logfile","Julius logfile (default=(null) : no log file)",(const char*)nullptr);
    ...
    ct->setField("juliusResultRecp","component(s) to send 'juliusResult' messages to (use , to separate multiple recepients), leave blank (nullptr) to not send any messages. The 'juliusResult' message contains the full julius result package, which includes the full word string, alignments (if available) and the word scores as well as am and lm scores.",(const char *) nullptr);
    ct->setField("kwsResultRecp","component(s) to send 'asrKeywordOutput' messages to (use , to separate multiple recepients), leave blank (nullptr) to not send any messages. The keyword result package only contains selected keywords (given in 'keywordList') from the full julius result package.",(const char *) nullptr);
    ct->setField("keywordList","name of text file to load keyword list from. The file must contain the keywords (all uppercase) sorted in alphabetical order, each keyword on a line.",(const char*)nullptr);
    ct->setField("excludeWords","array of strings (words) to exclude from result packages. Typically you would use this to exclude sentence start and end marks such as <s> or </s>. The default is an empty list, i.e. nothing will be excluded.",(const char *)nullptr,ARRAY_TYPE);
...
curVidx(0), vIdxStart(0), vIdxEnd(0), wst(0), writelen(0), juliusResultRecp(nullptr), kwsResultRecp(nullptr), period((FLOAT_DMEM)0.0),
turnStartSmileTimeLast((FLOAT_DMEM)0.0), turnStartSmileTime((FLOAT_DMEM)0.0), turnStartSmileTimeCur((FLOAT_DMEM)0.0),
decoderThread(nullptr), running(0),
sendKwsResult(0), sendJuliusResult(0), numKw(0),
keywordList(nullptr), keywords(nullptr),
excludeWords(nullptr), nExclude(0),
purgeQue(0), midx(0)
{
...
void cJuliusSink::fetchConfig()
{
  ...
  if (logfile != nullptr) { SMILE_IDBG(2,"Julius logfile = '%s'",logfile); }

  keywordList = getStr("keywordList");
  if (keywordList != nullptr) { SMILE_IDBG(2,"keyword selection list to use = '%s'",keywordList); }

  juliusResultRecp = getStr("juliusResultRecp");
  SMILE_IDBG(2,"juliusResultRecp = '%s'",juliusResultRecp);
  if (juliusResultRecp != nullptr) sendJuliusResult = 1;

  kwsResultRecp = getStr("kwsResultRecp");
  SMILE_IDBG(2,"kwsResultRecp = '%s'",kwsResultRecp);
  if (kwsResultRecp != nullptr) sendKwsResult = 1;
...
int external_fv_read_loader(void * cl, float *vec, int n) 
{
  ...
  if (cl!=nullptr) return ((cJuliusSink*)cl)->getFv(vec, n);
  else return -1; 
}
...
static void result_pass2_loader(Recog *recog, void *dummy)
{
  ...
  if (cl!=nullptr) cl->cbResultPass2(recog, dummy);
}
...
{
  cJuliusSink * cl = (cJuliusSink *)(recog->hook);
  if (cl!=nullptr) cl->cbStatusPass1Begin(recog, dummy);
}

static void result_pass1_loader(Recog *recog, void *dummy)
{
  cJuliusSink * cl = (cJuliusSink *)(recog->hook);
  if (cl!=nullptr) cl->cbResultPass1(recog, dummy);
}

void result_pass1_current_loader(Recog *recog, void *dummy)
{
  cJuliusSink * cl = (cJuliusSink *)(recog->hook);
  if (cl!=nullptr) cl->cbResultPass1Current(recog, dummy);
}
...
//  int n;
  ...
  if ((r->config->annotate.align_result_word_flag)&&(s->word_num>0)) {
    ...
    if (s->align == nullptr) //result_align_free(s->align);
      s->align = now;
    else {
...
int cJuliusSink::excludeWord(const char *w) 
{
  ...
  for (i=0; i<nExclude; i++) {
    ...
    if ((excludeWords[i] != nullptr) && (!strcmp(w,excludeWords[i]))) return 1;
  }
  return 0;
}
...
{
  juliusResult * res = nullptr;
  static char buf[MAX_HMMNAME_LEN];
  int i,wi;

  if (s->word != nullptr && s->word_num > 0) {
...
void cJuliusSink::loadKeywordList()
{
  ...
  if (keywordList != nullptr) {
    ...
    if (kwl != nullptr) {
      char *line=nullptr; size_t len=0; size_t r=-1;
      r = smile_getline(&line, &len, kwl);
      if ((r != (size_t)-1)&&(line!=nullptr)) {
        char *tmp=nullptr;
...
int cJuliusSink::tagKeywords(juliusResult *r)
{
  ...
  if ((keywordList != nullptr)&&(keywords != nullptr)) {
    int i; 
    for (i=0; i<r->numW; i++) {
    ...
    for (i=0; i<r->numW; i++) {
      if (r->wordFlag[i] & JULIUSRESULT_FLAG_KEYWORD) {
        if (kwc != i) { // copy to left...
          ...
          if (r->word[kwc] != nullptr) free(r->word[kwc]);
          r->word[kwc] = r->word[i]; r->word[i] = nullptr;
          if (r->phstr[kwc] != nullptr) free(r->phstr[kwc]);
          r->phstr[kwc] = r->phstr[i]; r->phstr[i] = nullptr;
        }
        kwc++;
      }
    }
    for (i=kwc; i<r->numW; i++) {
      if (r->word[i] != nullptr) free(r->word[i]);
      r->word[i] = nullptr;
      if (r->phstr[i] != nullptr) free(r->phstr[i]);
      r->phstr[i] = nullptr;
    }
    r->numW = kwc;
  }
  ...
  try {
    ...
    */
    ...
    if (j_final_fusion(recog) == FALSE) {
      Rprintf( "ERROR: Error while setup work area for recognition\n");
      j_recog_free(recog);
...
void cJuliusSink::juliusDecoderThread()
{
  ...
  int ret = j_open_stream(recog, nullptr);
  switch(ret) {
      case 0:			/* succeeded */
        break;
      case -1:      		/* error */
      ...
      case -2:			/* end of recognition process */
        if (jconf->input.speech_input == SP_RAWFILE) {
          //          Rprintf( "%d files processed\n", file_counter);
        } else if (jconf->input.speech_input == SP_STDIN) {
          Rprintf( "reached end of input on stdin\n");
        } else {
          Rprintf( "failed to begin input stream\n");
        }
        return;
  }
...
cJuliusSink::~cJuliusSink()
{
  ...
  if (!(decoderThread == nullptr)) smileThreadJoin(decoderThread);
  ...
  if (excludeWords != nullptr) free(excludeWords);
  if (keywords != nullptr) {
    for (i=0; i<numKw; i++) {
      if (keywords[i] != nullptr) free(keywords[i]);
    }
    free(keywords);
  }
}
