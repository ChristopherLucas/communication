...
SMILECOMPONENT_REGCOMP(cArffSink)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN_BEGIN
    ...
    ct->setField("errorOnNoOutput",(const char*)nullptr,1);
...
cArffSink::cArffSink(const char *_name) :
  ...
  nClasses(0),
  inr(0),
  classname(nullptr), classtype(nullptr),
  targetall(nullptr), targetinst(nullptr),
  disabledSink_(false)
{
}
...
void cArffSink::fetchConfig()
{
  ...
  for (i=0; i<nClasses; i++) {
    ...
    if (tmp!=nullptr) classname[i] = strdup(tmp);
    tmp = getStr_f(myvprint("class[%i].type",i));
    if (tmp!=nullptr) classtype[i] = strdup(tmp);
  }
/*
    ConfigType * classType = new ConfigType("arffClass");
...
*/
  ...
  if (getArraySize("target") != nClasses) {
    ...
    for (i=0; i<nClasses; i++) {
      ...
      if (t!=nullptr) targetall[i] = strdup(t);
      ...
      if (nInst > 0) {
        ...
        for (j=0; j<nInst; j++) {
          ...
          if (t!=nullptr) targetinst[i][j] = strdup(t);
        }
      }
      free(tmp);
    }
...
int cArffSink::myFinaliseInstance()
{
  ...
  if (instanceNameFromMetadata) {
    ...
    if ((instanceBase!=nullptr)&&(strlen(instanceBase)>0)&&
  ...
  if (!ap) {
    ...
    if (nClasses > 0) {
      for (i=0; i<nClasses; i++) {
        if (classtype[i] == nullptr) fprintf(filehandle, "@attribute %s numeric%s",classname[i],NEWLINE);
...
int cArffSink::myTick(long long t)
{
  ...
  // classes: 
  if ((vec->tmeta->metadata.iData[1] == 1234)&&(vec->tmeta->metadata.custom != nullptr)&&(vec->tmeta->metadata.customLength > 0)&&(useTargetsFromMetadata)) {
    ...
    if (nClasses > 0) {
      if (nInst>0) {
        if (inr >= nInst) {
          ...
          if (targetall != nullptr) {
            for (i=0; i<nClasses; i++) {
              if (targetall[i] != nullptr)
                fprintf(filehandle,",%s",targetall[i]);
              else
                fprintf(filehandle,",nullptr");
            }
          } else {
            for (i=0; i<nClasses; i++) {
              fprintf(filehandle,",nullptr");
...
cArffSink::~cArffSink()
{
  ...
  if (classname!=nullptr) {
    for (i=0; i<nClasses; i++) if (classname[i] != nullptr) free(classname[i]);
    free(classname);
  }
  if (classtype!=nullptr) {
    for (i=0; i<nClasses; i++) if (classtype[i] != nullptr) free(classtype[i]);
    free(classtype);
  }
  if (targetall!=nullptr) {
    for (i=0; i<nClasses; i++) if (targetall[i] != nullptr) free(targetall[i]);
    free(targetall);
  }
  if (targetinst!=nullptr) {
    int j;
    for (i=0; i<nClasses; i++) {
      if (targetinst[i] != nullptr) {
        for (j=0; j<nInst; j++) {
          if (targetinst[i][j] != nullptr) free(targetinst[i][j]);
        }
        free(targetinst[i]);
      }
    }
