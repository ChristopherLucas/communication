...
int cWaveSink::myFinaliseInstance()
{
  ...
  if (fHandle == nullptr) {
    ...
    if (fHandle == nullptr) COMP_ERR("failed to open output file '%s'",filename);
  }
...
int cWaveSink::writeData(cMatrix *m) 
{
  if (m!=nullptr) {
    if (m->N != nChannels) { SMILE_IERR(1,"number of chanels is inconsistent! %i <-> %i",m->fmeta->N,nChannels); return 0; }
      ...
      // convert data
      long i;
      if ((m->nT > sampleBufferLen)&&(sampleBuffer!=nullptr)) free(sampleBuffer);
      sampleBufferLen = m->nT;
      if (sampleBuffer == nullptr) sampleBuffer = malloc(nBytesPerSample*nChannels*m->nT);
