...
SMILECOMPONENT_REGCOMP(cSignalGenerator)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ...
    ct->setField("fieldNames","An array of names of fields in the output vector (default for all: noiseN, where N is the field number).",(const char*)nullptr,ARRAY_TYPE);
...
cSignalGenerator::cSignalGenerator(const char *_name) :
  ...
  fieldNames(nullptr), nElements(nullptr), samplePeriod(1.0), lastP(0.0), val(0.0), scale(1.0), myt(0), curT(0)
{
  // ...
}
...
cSignalGenerator::~cSignalGenerator()
{
  if (nFields > 0) {
    ...
    if (fieldNames != nullptr) {
      for (i=0; i<nFields; i++) {
        if (fieldNames[i] != nullptr) free(fieldNames[i]);
      }
      free(fieldNames);
    }
  }
