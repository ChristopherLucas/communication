...
cSpecScale::cSpecScale(const char *_name) :
  cVectorProcessor(_name),
    ...
    f_t(nullptr), spline_work(nullptr), 
    y(nullptr), y2(nullptr), audw(nullptr)
{
...
int cSpecScale::dataProcessorCustomFinalise()
{
  ...
  if (f_t == nullptr) f_t = (double*)malloc(sizeof(double)*nMag);
...
int cSpecScale::processVectorFloat(const FLOAT_DMEM *src, FLOAT_DMEM *dst, long Nsrc, long Ndst, int idxi) // idxi=input field index
{
  ...
  if (y == nullptr) y = (double *)malloc(sizeof(double)*nMag);
  if (y2 == nullptr) y2 = (double *)malloc(sizeof(double)*nMag);
...
{
  if (y != nullptr) free(y);
  if (y2 != nullptr) free(y2);

  if (audw != nullptr) free(audw);
  if (spline_work != nullptr) free(spline_work);
  if (f_t != nullptr) free(f_t);
}
