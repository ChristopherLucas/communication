...
cSmileResample::cSmileResample(const char *_name) :
  ...
  inputBuf(nullptr), outputBuf(nullptr), lastOutputBuf(nullptr),
  resampleWork(nullptr), rowout(nullptr), matnew(nullptr), row(nullptr),
  flushed(0)
{
}
...
int cSmileResample::myTick(long long t)
{
  ...
  if (mat != nullptr) {
    ...
    if (useQuickAlgo) {
      ...
      if (matnew == nullptr) matnew = new cMatrix(mat->N, winSizeFramesTarget,DMEM_FLOAT);
      //if (rowout == nullptr) rowout = new cMatrix(1,winSizeFramesTarget/2, DMEM_FLOAT);
      //if (row == nullptr) row = new cMatrix(1,winSizeFrames, DMEM_FLOAT);
      long i,j,n,r=0;
    ...
    /*
    if (row == nullptr) row = new cMatrix(1,mat->nT, mat->type);
*/
    if (matnew == nullptr) matnew = new cMatrix(mat->N, winSizeFramesTarget/2,DMEM_FLOAT);
    if (rowout == nullptr) rowout = new cMatrix(1,winSizeFramesTarget/2, DMEM_FLOAT);
    if (row == nullptr) row = new cMatrix(1,winSizeFrames, DMEM_FLOAT);
    ...
    for (i=0; i<mat->N; i++)  {
      ...
      if (rowr==nullptr) COMP_ERR("cWindowProcessor::myTick : Error getting row %i from matrix! (return obj = nullptr!)",i);
...
//         printf("WINPROC '%s' mat==nullptr tickNr=%i EOI=%i\n",getInstName(),t,isEOI());
    return 0;
  }
...
{
  if (inputBuf != nullptr) free(inputBuf);
  if (outputBuf != nullptr) free(outputBuf);
  if (lastOutputBuf != nullptr) free(lastOutputBuf);
  if (row != nullptr) delete row;
  if (rowout != nullptr) delete rowout;
  if (matnew != nullptr) delete matnew;
