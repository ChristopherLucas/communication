...
SMILECOMPONENT_REGCOMP(cPortaudioWavplayer)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ...
    ct->setField("wavefile","The filenames of the wavesample(s) to play (triggered via smile message or numbers read from the input level).",(const char*)nullptr,ARRAY_TYPE);
    ct->setField("keyword","List of keywords or classnames (in same order as wave files)",(const char*)nullptr,ARRAY_TYPE);
    ct->setField("indices","List of class indicies for classification results (same order as wave files)",-1.0,ARRAY_TYPE);

    ct->setField("semaineCallbackRcpt","Reciepient of SEMAINE callback message for echo feedback supression (start speaking / stop speaking messages, i.e. when player starts playing/stops playing).",(const char*)nullptr);
...
int cPortaudioWavplayer::myConfigureInstance()
{
  ...
  if (!wavsLoaded) {
    ...
    for (i=0; i<nWavs; i++) {
      ...
      if (filehandle == nullptr) COMP_ERR("failed to open wave file '%s'",filename);
...
//   and, do not call Pa_terminate while the callback is running... i.e. introduce a callback mutex!
  ...
  if (wavelength != nullptr) { free(wavelength); }

  if (keywords != nullptr) free(keywords);
