...
SMILECOMPONENT_REGCOMP(cPortaudioSource)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ...
    ct->setField("period",(const char*)nullptr,0,0,0);
...
int cPortaudioSource::myConfigureInstance()
{
  ...
  if (!isPaInit) {
    SMILE_MSG(0,"Before Pa_Initialize");    
...
#endif
  SMILE_MSG(0,"numDevices = %i", numDevices); 
  auto defaultDevice = Pa_GetDefaultInputDevice();
  SMILE_MSG(0,"defaultDevice = %i", defaultDevice);   
  // list devices...
  if (listDevices) {
    if (listDevices != -1) {
      printDeviceList();
...
//   and, do not call Pa_terminate while the callback is running... i.e. introduce a callback mutex!
  ...
  if (callbackMatrix != nullptr) delete callbackMatrix;
}
...
//    obj->bufWrapper  = pcmBufferMakeWrapper( obj->bufWrapper, 0, 1, SAMPLETYPE_I16, 0, 0, nullptr);
