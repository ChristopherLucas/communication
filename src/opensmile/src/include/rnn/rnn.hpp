...
#define RNN_ERR(level, ...) { Rprintf( "RNN ERROR: "); Rprintf( __VA_ARGS__); Rprintf("\n"); }
...
public:
  ...
  virtual const FLOAT_NN *forward(const FLOAT_NN *x, long *N=nullptr)=0;

  // get the current output activation
  virtual const FLOAT_NN *getOutput(long *N=nullptr) { if(N!=nullptr) *N=1; return &cellOutput; }
...
public:
  cNnNNcell(cNnTf *_transfer=nullptr, long _cIdx=0, long _lIdx=0) :
    cNnCell(_transfer, _cIdx, _lIdx)
  {}

  // feed data through the cell and compute the cell output (return value)
  // note: N and the size of *x must match the value returned by getInputSize()
  virtual const FLOAT_NN *forward(const FLOAT_NN *x, long *N=nullptr) 
  {
...
public:
  ...
  ~cNnLSTMcell() {
    //if (transferOut != nullptr) delete transferOut;
    //if (transferGate != nullptr) delete transferGate;
    if (sc != nullptr) free(sc);
    if (cellOutputs != nullptr) free(cellOutputs);
    if (peep != nullptr) free(peep);
  }
};
...
protected:
  ...
  {
    if ((cell!=nullptr) && (cell[0] != nullptr)) {
...
public:
  cNnNNlayer(long _nCells, int _layerIdx=0, int _direction=0, long _nContext=0) : cNnLayer(_nCells, _layerIdx, _direction, _nContext), _tf(nullptr) {}
...
public:
  ...
  ~cNnLSTMlayer() {
    if (_tf != nullptr) delete _tf;
    if (_tfO != nullptr) delete _tfO;
    if (_tfG != nullptr) delete _tfG;
  }
};
...
public:
  ...
  {
    if (inputStart!=nullptr) free(inputStart);
    if (outputs != nullptr) free(outputs);
    if (bias != nullptr) free(bias);
...
public:
  ...
  {
    if ((idxT >= 0)&&(idxT<=nConnections)&&(idxF_local >= 0)&&(connection[idxT] != nullptr))
      connection[idxT]->setWeights(x,N,idxF_local);
  }

  void setBias(int idxT, FLOAT_NN *x, long N)
  {
    if ((idxT >= 0)&&(idxT<=nConnections)&&(connection[idxT] != nullptr))
  ...
  virtual ~cNnRnn() {
    ...
    if (layer != nullptr) {
      for (i=0; i<nLayers; i++) {
        if (layer[i] != nullptr) delete layer[i];
      }
      free(layer);
    }
...
public:
  ...
  ~cRnnNetFile() {
    int i;
    for (i=0; i<4*MAX_LAYERS; i++) { if (wv[i] != nullptr) delete wv[i]; }
  }
};
