...
struct sTfData {
  ...
  double * userData;  /* this can be nullptr if no user data is present, i.e. when nUserdata = 0 */
  double * vectors;   /* this can be nullptr if only userData is present and no vectors (nVec = 0) */
  /* NOTE: at least one of the two above pointers must be != nullptr */
