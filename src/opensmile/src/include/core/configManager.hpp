...
class  ConfigValue {
  ...
  public:
    ...
    virtual const char* getStr(int n=0) const { return nullptr; }
    virtual char getChar(int n=0) const { return 0; }
    virtual ConfigInstance* getObj(int n=0) const { return nullptr; }
    virtual ConfigValue* getValue(int n=0) const { return nullptr; }
    ...
    virtual int updateWith(const ConfigValue *val) {
      if ((val!=nullptr)&&(val->isSet())) { copyFrom(val); return 1; }  
      else { return 0; }
    }

    virtual int findField(const char *_name=nullptr, int noerr=0) const { return -1; }
    virtual int findFieldWr(const char *_name=nullptr) { return -1; }

    virtual int getType() const { return type; }
    virtual int isSet(int n=-1) const { return set; }
    virtual int getN() const { return -1; }
    virtual int getSize() const { return -1; }
    virtual char ** getAAkeys() const { return nullptr; }  // get keys of associative array fields
...
class  ConfigValueStr : public ConfigValue {
  ...
  public:
    ConfigValueStr() : str(nullptr), N(0) { type = CFTP_STR; set = 0; }
    ConfigValueStr(const char * value) : str(nullptr), N(0) {
      type = CFTP_STR;
      setValue(value);
    }
    const char * getStr(int n=0) const { return str; }
    char getChar(int n=0) const { if (str!=nullptr) return str[0]; else return 0; }

    void setValue(const char * value, int n=0) {
      if (str != nullptr) { free(str); str=nullptr; }
      if (value==nullptr) { set = 0; return; }
      str = strdup(value); N=strlen(value);
      set=1;
    }
    void copyFrom(const ConfigValue *val);

    ~ConfigValueStr() { if (str != nullptr) free(str); }
};
...
class  ConfigValueObj : public ConfigValue {
  ...
  public:
    ConfigValueObj() : obj(nullptr), freeObj(0) { type = CFTP_OBJ; set = 0; }
...
//    void copyFrom(const ConfigValue *val, int arrIdx);  // copy single element ??
    ...
    virtual int isSet(int n=-1) const {
      if (n<0) return set;
      else { if ((n<=maxN)&&(el[n]!=nullptr)&&(el[n]->isSet())) return 1; else return 0; }
    }
};
...
class  ConfigValueNumArr : public ConfigValueArr {
  public:
    ...
    virtual int getInt(int n=0) const {
      if (checkIdx(n)&&(el[n] != nullptr)) return ((ConfigValueNum *)(el[n]))->getInt();  // get value via () operator
      else return 0;
    }

    virtual double getDouble(int n=0) const {
      if (checkIdx(n)&&(el[n] != nullptr)) return ((ConfigValueNum *)(el[n]))->getDouble();  // get value via () operator
      else return 0.0;
    }
...
class  ConfigType {
  ...
  public:
    ...
    ConfigType( ConfigType const& copy, const char *_newname=nullptr);
    ...
    char * getDfltStr(int n) const { if ((n<I)&&(n>=0)) return element[n].dfltStr; else return nullptr; }
    char getDfltChr(int n) const { if ((n<I)&&(n>=0)) return element[n].dfltChar; else return 0; }
    const ConfigType * getDfltObj(int n) const { if ((n<I)&&(n>=0)) return element[n].subType;  else return nullptr; }
    
    // get index of field _name, and optionally an array index specified in _name
    int findField(const char *_name, int *arrI=nullptr, char **arrS=nullptr) const;
    ...
    void printTypeHelp(char *basestr=nullptr, int _subtype=1) const;  
    ...
    void printTypeDfltConfig(char *basestr=nullptr, int _subtype=1, int withDescription=0, int ignInternal=2) const; 
...
class  ConfigInstance {
  ...
  public:
    ...
    /* either N or name may be specified. if not specified name=nullptr and N=-1 */
    void setValue(ConfigValue *val, int n=-1, const char *_name=nullptr, int arrIdx=-1);   /* recursively dereference subobj pointers */
    void setValue(const char *_name, ConfigValue *val) {
      setValue(val,-1,_name);
    }
    const ConfigValue *getValue(int n=-1, const char *_name=nullptr, int arrIdx=-1) const; /* recursively dereference subobj pointers */
    ...
    int getInt(const char *_name) { const ConfigValue *r = getValue(-1,_name); if (r!=nullptr) return r->getInt(); else return 0; }
    double getDouble(const char *_name) { const ConfigValue *r = getValue(-1,_name); if (r!=nullptr) return r->getDouble(); else return 0; }
    const char* getStr(const char *_name) { const ConfigValue *r = getValue(-1,_name); if (r!=nullptr) return r->getStr(); else return 0; }
    char getChar(const char *_name) { const ConfigValue *r = getValue(-1,_name); if (r!=nullptr) return r->getChar(); else return 0; }
    ConfigInstance * getObj(const char *_name) { const ConfigValue *r = getValue(-1,_name); if (r!=nullptr) return r->getObj(); else return 0; }
    ...
    void setInt(const char *_name, int value) { ConfigValue *r = new ConfigValueNum(value); if (r!=nullptr) setValue(_name,r); }
    void setDouble(const char *_name, double value) { ConfigValue *r = new ConfigValueNum(value); if (r!=nullptr) setValue(_name,r); }
    void setStr(const char *_name, const char * value) { ConfigValue *r = new ConfigValueStr(value); if (r!=nullptr) setValue(_name,r); }
    void setChar(const char *_name, char value) { ConfigValue *r = new ConfigValueChr(value); if (r!=nullptr) setValue(_name,r); }
    void setObj(const char *_name, ConfigInstance *value) { ConfigValue *r = new ConfigValueObj(value); if (r!=nullptr) setValue(_name,r); }
...
class  cConfigReader {
  protected:
    ...
    char *lastLevelFile;  // config file that was previously included (can be pointer identical to inputPath or nullptr in this case)
    int  inputId;
    cCommandlineParser *cmdparser;
    
  public:
    cConfigReader(const char *_inputPath, int _inputId=-1, cCommandlineParser *cmdparser_=nullptr);
    void setCmdparser(cCommandlineParser *_parser) { cmdparser = _parser; }
    virtual void setId(int _id) { if (_id>=0) inputId = _id; }
    virtual int openInput(const char*name, int *idx0=nullptr) { return 0; }
    ...
    virtual ConfigInstance *getInstance(const char *_instname, const ConfigType *_type, cConfigManager *cman=nullptr)=0;    /* get an instance with name _instname */
...
class  cFileConfigReader : public cConfigReader {
  ...
  public:
    cFileConfigReader(const char *filename, int id=-1, cCommandlineParser *cmdparser_=nullptr) : cConfigReader(filename,id,cmdparser_), inst(nullptr), nInst(0), nInstAlloc(0) { openInput(); }
    virtual int openInput(const char*name=nullptr, int *idx0=nullptr);
    virtual int closeInput() { return 0; }

    virtual char ** findInstancesByTypeName(const char *_typename, int *N);  /* returns names (and no.) of instances of type "typename" */
    virtual ConfigInstance *getInstance(const char *_instname, const ConfigType *_type, cConfigManager *cman=nullptr);    /* get an instance with name _instname */
...
class  cConfigManager {
  ...
  public:
    cConfigManager(cCommandlineParser *_parser = nullptr);
    ...
    /* get keys of an associative array, and optionally the number N of elements
       NOTE: some keys in the returned array might be nullptr if the element was not set!! */
    char **getArrayKeys(const char *_name, int *N=nullptr) const;
    /* the _f getXXX functions free the memory allocated for _name before they return normally! */
    char **getArrayKeys_f(char *_name, int *N=nullptr) const {  /* return value is read only! */
    ...
    /* the _f getXXX functions free the memory allocated for _name before they return normally! */
    const int getInt_f(char *_name, int *isSet=nullptr) const {  /* return value is read only! */
      int d = getInt(_name, isSet);
      if (_name != nullptr) free(_name);
      return d;
    }

    const double getDouble(const char *_name, int *isSet=nullptr) const {
      const ConfigValue *v = getValue(_name);
      if (v!=nullptr) {
    ...
    /* the _f getXXX functions free the memory allocated for _name before they return normally! */
    const double getDouble_f(char *_name, int *isSet=nullptr) const {  /* return value is read only! */
      double d = getDouble(_name, isSet);
      if (_name != nullptr) free(_name);
      return d;
    }

    const char    * getStr(const char *_name, int *isSet=nullptr) const {  /* return value is read only! */
      const ConfigValue *v = getValue(_name);
      if (v!=nullptr) {
    ...
    /* the _f getXXX functions free the memory allocated for _name before they return normally! */
    const char    * getStr_f(char *_name, int *isSet=nullptr) const {  /* return value is read only! */
      const char *s = getStr(_name, isSet);
      if (_name != nullptr) free(_name);
      return s;
    }
    
    const char getChar(const char *_name, int *isSet=nullptr) const {
      const ConfigValue *v = getValue(_name);
      if (v!=nullptr) {
    ...
    /* the _f getXXX functions free the memory allocated for _name before they return normally! */
    const char    getChar_f(char *_name, int *isSet=nullptr) const {  /* return value is read only! */
    ...
    int isSet(const char *_name) const {
      ...
      } catch (cConfigException &e) { // removed &c ??
        return 0;
      }
    }
    ...
    /* print help for all registered config types, if selection==nullptr, 
       else print help for type(s) beginning with the name given in *selection */
    void printTypeHelp(int _subtype=1, const char *selection=nullptr, int dfltConf=0);

    /* print a default config file section for the type given in selection, 
       or for all types if selection == nullptr. 
       If _subtype == 1, explicitely print default values of all subtypes. */
    void printTypeDfltConfig(const char *selection=nullptr, int _subtype=1, int fullMode=0, int withDescription=1);
    ...
    void * getExternalPointer(const char * name) {
      if (externalObjectMap_->find(name) != externalObjectMap_->end()) {
        ...
        SMILE_WRN(1, "configManager: external pointer with name '%s' not found! Returning nullptr.", name);
      }
      return nullptr;
    }
    /*
