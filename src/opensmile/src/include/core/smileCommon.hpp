...
//#ifdef __WINDOWS || defined _WIN32
#if defined(WIN32)
...
#define SMILE_THREAD_RET return nullptr

#define smileThread  pthread_t
#define smileThreadCreate(THRD,T_MAIN,ARG) (pthread_create(&(THRD), nullptr, T_MAIN, ARG)==0)
#define smileThreadJoinWret(THRD,RET) pthread_join(THRD,&RET)
#define smileThreadJoin(THRD) pthread_join(THRD,nullptr)

#define smileMutex pthread_mutex_t
#define smileMutexInitVar(mtx) mtx = PTHREAD_MUTEX_INITIALIZER
#define smileMutexCreate(mtx) pthread_mutex_init(&(mtx), nullptr)
...
#define smileCondCreate(COND) { pthread_mutex_init(&(COND.mtx), nullptr); \
                                pthread_cond_init(&(COND.cond), nullptr); COND.signaled = 0; }
...
#define smileThreadCreate(THRD,T_MAIN,ARG) ((void *)(THRD = (HANDLE)CreateThread(nullptr,0,(LPTHREAD_START_ROUTINE)T_MAIN,ARG,0,nullptr)) != nullptr)
//#define smileThreadJoinWret(THRD,RET) pthread_join(THRD,&RET)
...
//#define smileMutexCreate(mtx) ( (mtx = CreateMutex( nullptr, FALSE, nullptr )) != nullptr )
...
pthread_cond_init (pthread_cond_t *cv)
{
  ...
  cv->event_ = CreateEvent (nullptr,  // no security
                            TRUE,  // manual-reset
                            FALSE, // non-signaled initially
                            nullptr); // unnamed
