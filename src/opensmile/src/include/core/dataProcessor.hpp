...
class  cDataProcessor : public cSmileComponent {
  ...
  protected:
    ...
    int copyInputName_; /* 1=copy input name and append "nameAppend" (if != nullptr) , 0 = do not copy and set name only to "nameAppend" */
    ...
    // (optional) fullName = 1: match full field name instead of part of name
    long findField(const char *namePartial, int fullName=0, long *N=nullptr, const char **fieldName=nullptr, long nEl=-1, int *more=nullptr, int *fieldIdx=nullptr);
    ...
    //
    long findElement(const char *namePartial, int fullName, long *N, const char **fieldName, int *more, int *fieldIdx=nullptr);
    ...
    virtual void setEOI() {
      ...
      if (reader_!=nullptr) reader_->setEOI();
      if (writer_!=nullptr) writer_->setEOI();
    }

    virtual void unsetEOI() {
      cSmileComponent::unsetEOI();
      if (reader_!=nullptr) reader_->unsetEOI();
      if (writer_!=nullptr) writer_->unsetEOI();
    }
