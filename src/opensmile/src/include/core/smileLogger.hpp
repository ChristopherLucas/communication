...
class  cSmileLogger {
  ...
  public:
    cSmileLogger(int _loglevel=0, const char *_logfile=nullptr, int _append=0);  // logfile == nullptr: output to stderr
                                                             // loglevel is the OVERALL loglevel
    cSmileLogger(int loglevel_msg, int loglevel_wrn, int loglevel_err, int loglevel_dbg, const char *logfile=nullptr, int _append=0);
    ~cSmileLogger();
    ...
    */
		void log(int type, char *s, int level=0, const char *module=nullptr)
		      { logMsg(type,s,level,module); }

		void print(char *s, int level=0)
		  { logMsg(LOG_PRINT,s,level,nullptr); }

		void message(char *s, int level=0, const char *module=nullptr)
		  { logMsg(LOG_MESSAGE,s,level,module); }

		void warning_smile(char *s, int level=0, const char *module=nullptr)
		  { logMsg(LOG_WARNING,s,level,module); }

		void error_smile(char *s, int level=0, const char *module=nullptr)
		  { logMsg(LOG_ERROR,s,level,module); }

    #ifdef DEBUG
    void debug(char *s, int level=0, const char *module=nullptr)
      { logMsg(LOG_DEBUG,s,level,module); }
    #else
    void debug(char *s, int level=0, const char *module=nullptr) { if (s!=nullptr) free(s); }
    #endif
};
...
extern  cSmileLogger SMILE_LOG_GLOBAL;
...
#define SMILE_ERR(level, ...) { if (level <= SMILE_LOG_GLOBAL.getLogLevel_err()) SMILE_LOG_GLOBAL.error_smile(FMT(__VA_ARGS__), level, MODULE); }
#define SMILE_IERR(level, ...) { if (level <= SMILE_LOG_GLOBAL.getLogLevel_err()) { char *__mm = FMT("instance '%s'",getInstName()); SMILE_LOG_GLOBAL.error_smile(FMT(__VA_ARGS__), level, __mm); free(__mm); } }

#define SMILE_WRN(level, ...) { if (level <= SMILE_LOG_GLOBAL.getLogLevel_wrn()) SMILE_LOG_GLOBAL.warning_smile(FMT(__VA_ARGS__), level, MODULE); }
#define SMILE_IWRN(level, ...) { if (level <= SMILE_LOG_GLOBAL.getLogLevel_wrn()) { char *__mm = FMT("instance '%s'",getInstName()); SMILE_LOG_GLOBAL.warning_smile(FMT(__VA_ARGS__), level, __mm); free(__mm); } }
