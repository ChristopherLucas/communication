...
class  cDataReader : public cSmileComponent {
  ...
  public:
    ...
    // absolute
    cVector * getFrame(long vIdx, int special=-1, int privateVec=0, int *result=nullptr);
    cMatrix * getMatrix(long vIdx, long length, int special=-1, int privateVec=0); // vIdx: start index of matrix (absolute)
    
    // relative:
    cVector * getFrameRel(long vIdxRelE, int privateVec=0, int noInc=0, int *result=nullptr);
    ...
    cVector * getNextFrame(int privateVec=0, int *result=nullptr);
    ...
    */
    ...
    const char * getFieldName(int n, int *N=nullptr, int *arrNameOffset=nullptr)    // name of field n
    { 
        if ((n >= 0) && (n < myLcfg->Nf)) {
