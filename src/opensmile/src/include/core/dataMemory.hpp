...
class  FieldMetaInfo {
  ...
  FieldMetaInfo() : name(nullptr), N(0), dataType(0), infoSize(0), info(nullptr), arrNameOffset(0) {}
  void copyFrom(FieldMetaInfo *f) {
    if (f!=nullptr) {
      ...
      if (f->name != nullptr) name = strdup(f->name);
      else name = nullptr;
    }
  }
  ~FieldMetaInfo() { 
...
class  cVectorMeta { public:
  ...
  char * text; /* custom meta-data text, will be auto-freed using free() if not nullptr */
  void * custom; /* custom meta data, will be auto-freed using free() if not nullptr */
  ...
  cVectorMeta() : ID(0), text(nullptr), custom(nullptr), customLength(0)
  {
  ...
  void cloneFrom(const cVectorMeta *v)
  {
    if (v!=nullptr) {
      ...
      if ((v->customLength > 0)&&(v->custom != nullptr)) {
        if ((custom != nullptr)&&(customLength>0)) free(custom);
  ...
  ~cVectorMeta() { 
    if (text != nullptr) free(text); 
    if ((customLength>0)&&(custom != nullptr)) free(custom); 
  }
};
...
class  FrameMetaInfo {
  public:
    ...
    FrameMetaInfo() : N(0), Ne(0), field(nullptr) 
    {
    ...
    // if more != nullptr, then *more (if > 0) will be used as a start field index for the search (i>=*more)
    //                  and *more will be filled with the number of additional matches found (0 if the returned field is the only match)
    int findField(const char*_fieldName, int *arrIdx=nullptr, int *more=nullptr) const;
    ...
    //   if more != nullptr, then *more (if > 0) will be used as a start field index for the search (i>=*more)
    //                    and *more will be filled with the number of additional matches found (0 if the returned field is the only match)
    int findFieldByPartialName(const char*_fieldNamePart, int *arrIdx=nullptr, int *more=nullptr) const;

    long fieldToElementIdx(long _field, long _arrIdx=0) const;
    long elementToFieldIdx(long _element, long *_arrIdx=nullptr) const;
...
class  TimeMetaInfoMinimal { public:
...
class  cVector { public:
  ...
  const char *name(int n, int *lN=nullptr);
  ...
  void tmetaReplace(TimeMetaInfoMinimal *xtmeta, double period = 0.0) {
    if ((tmeta_minimal != nullptr)&&(!tmetaAlien)) {
...
/*    long i;
  ...
  cMatrix * getRow(long R, cMatrix *r) {
    long i;
    if (r==nullptr) r = new cMatrix(1,nT,type);
    else {
      if (r->nT != nT) { delete r; r = new cMatrix(1,nT,type); }
      if (type != r->type) return nullptr;
    }
    long nn = MIN(nT,r->nT);
    if (type==DMEM_FLOAT) {
  ...
  cVector* getCol(long C, cVector *c) {
    if (c==nullptr) c = new cVector(N,type);
    long i;
    if (type==DMEM_FLOAT) for (i=0; i<N; i++) { c->dataF[i] = dataF[N*C+i]; }
    else if (type==DMEM_INT) for (i=0; i<N; i++) { c->dataI[i] = dataI[N*C+i]; }
    else { delete c; return nullptr; }
...
class  sDmLevelConfig { public:
  ...
  { 
    lenSec = T*(double)nT; 
    if (_name != nullptr) name = strdup(_name);
  }
...
//lcfg(_name, 0.0, 0.0, _nT, _type, rb),
  ...
  sDmLevelConfig(sDmLevelConfig const &orig) :
    ...
    fmeta(orig.fmeta),
    name(nullptr) { 
      if (orig.name != nullptr) name = strdup(orig.name); 
    }
  ...
  sDmLevelConfig(const char *_name, sDmLevelConfig &orig) :
    ...
    name(nullptr) { 
      if (_name != nullptr) name = strdup(_name); 
      else if (orig.name != nullptr) name = strdup(orig.name); 
    }
  ...
  ~sDmLevelConfig() { if (name != nullptr) free(name); }
};
...
*/
    ...
    // TODO: error codes
    long validateIdxRangeR(long actualVidx, long *vIdx, long vIdxEnd, int special=-1, int rdId=-1, int noUpd=0, int *padEnd=nullptr)
    {
  ...
  public:
    ...
    cDataMemoryLevel(int _levelId, sDmLevelConfig &cfg, const char *_name = nullptr) :
      myId(_levelId), _parent(nullptr),
      nCurRdr(0), writeReqFlag(0),
      lcfg(_name, cfg), fmetaNalloc(0),
      data(nullptr), curW(0), curR(0), 
      curRr(nullptr), nReaders(0), 
      tmeta(nullptr), EOI(0), EOIcondition(0)
    {
    ...
    cDataMemoryLevel(int _levelId, const char *_name, long _nT, int rb=1, int dyn=0, int _type=DMEM_FLOAT) :
      myId(_levelId), _parent(nullptr),
      nCurRdr(0), writeReqFlag(0),     
      lcfg(_name, 0.0, 0.0, _nT, _type, rb), fmetaNalloc(0),
        //sDmLevelConfig(const char *_name, double _T, double _frameSizeSec, long _nT=10, int _type=DMEM_FLOAT, int _isRb=1) :
      data(nullptr),  curW(0), curR(0),
      curRr(nullptr), nReaders(0),
      tmeta(nullptr), EOI(0), EOIcondition(0)
      //,RWptrMtx(nullptr), RWstatMtx(nullptr), RWmtx(nullptr),
    {
...
//      lcfg.fmeta = &fmeta;
    ...
    // *arrIdx (if not nullptr) will be set to the array index of the requested element, if it is in an array field
    ...
    // int special = special index, e.g. DMEM_IDX_CURR, DMEM_IDX_CURW
    // *result (if not nullptr) will contain a result code indicating success or reason of failure (left or right buffer margin exceeded, etc.)
    // rdId is the id of the current reader (or -1 for an unregistered or global reader)
    cVector * getFrame(long vIdx, int special=-1, int rdId=-1, int *result=nullptr);  
    cMatrix * getMatrix(long vIdx, long vIdxEnd, int special=-1, int rdId=-1, int *result=nullptr);  
    ...
    int checkRead(long vIdx, int special=-1, int rdId=-1, long len=1, int *result=nullptr) 
    {
      long rIdx;
...
//OBSOLETE:    long getLevelSize() { return lcfg.nT; } // get size of level in frames
    ...
    const char * getFieldName(int n, int *lN=nullptr, int *larrNameOffset=nullptr);
    ...
    ~cDataMemoryLevel() {
      ...
      //--------
      if (tmeta != nullptr) delete[] tmeta; // was: free(tmeta) !
      if (data != nullptr) delete data;
      if (curRr != nullptr) free(curRr);
    }
...
public:
  ...
  cSmileList(long _elSize=1, long _blockAlloc=50) : nEl(0), nElAlloc(0), blockAlloc(_blockAlloc), elSize(_elSize), el(nullptr)
    { if (elSize < 1) { elSize=1; } }
  ~cSmileList() { if (el!=nullptr) free(el); }
};
...
public:
  ...
  int findRequest(const char *lvl, const char *instName) {
    ...
    for (i=0; i<getNEl(); i++) {
      ...
      if (x!=nullptr) {
        if ( (!strcmp(lvl,x->levelName)) && ((instName==nullptr)||(!strcmp(instName,x->instanceName))) ) return i+1;
      }
    }
    return 0;
  }
...
class  cDataMemory : public cSmileComponent {
  ...
  protected:
    ...
    void fetchConfig() {}
    virtual int myRegisterInstance(int *runMe=nullptr);
  ...
  public:
    ...
    cDataMemory() : cSmileComponent("dataMemory"), level(nullptr), 
      nLevelsAlloc(0), nLevels(-1) {}

    cDataMemory(const char *_name) : cSmileComponent(_name), level(nullptr),
      nLevelsAlloc(0), nLevels(-1) {}

    /* register a read request (during "register" phase) */
    void registerReadRequest(const char *lvl, const char *componentInstName=nullptr);
    void registerWriteRequest(const char *lvl, const char *componentInstName=nullptr);
    ...
    int addLevel(sDmLevelConfig *_lcfg, const char *_name=nullptr);
    ...
    const sDmLevelConfig * queryReadConfig( int _level, long _blocksizeReader=1 )
      { if ((_level>=0)&&(_level<=nLevels)) return level[_level]->queryReadConfig(_blocksizeReader); else return nullptr; }
    const sDmLevelConfig * queryReadConfig( int _level, double _blocksizeReaderSeconds=1 )
      { if ((_level>=0)&&(_level<=nLevels)) return level[_level]->queryReadConfig(_blocksizeReaderSeconds); else return nullptr; }
  ...
  // OBOSOLETE : ???
    ...
    int setFrame(int _level, long vIdx, const cVector *vec, int special=-1/*, int *result=nullptr*/)
      { if ((_level>=0)&&(_level<=nLevels)) return level[_level]->setFrame(vIdx,vec,special/*, result*/); else return 0; }
    int setMatrix(int _level, long vIdx, const cMatrix *mat, int special=-1/*, int *result=nullptr*/)
    ...
    int checkRead(int _level, long vIdx, int special=-1, int rdId=-1, long len=1, int *result=nullptr)
    ...
    cVector * getFrame(int _level, long vIdx, int special=-1, int rdId=-1, int *result=nullptr)
      { if ((_level>=0)&&(_level<=nLevels)) return level[_level]->getFrame(vIdx,special,rdId,result); else return nullptr; }
    cMatrix * getMatrix(int _level, long vIdx, long vIdxEnd, int special=-1, int rdId=-1, int *result=nullptr)
      { if ((_level>=0)&&(_level<=nLevels)) return level[_level]->getMatrix(vIdx,vIdxEnd,special,rdId,result); else return nullptr; }
...
//      { if ((_level>=0)&&(_level<=nLevels)) return level[_level]->getLevelSize(); else return -1; }
    ...
    const sDmLevelConfig * getLevelConfig(int _level)
      { if ((_level>=0)&&(_level<=nLevels)) return level[_level]->getConfig(); else return nullptr; }
    
    const char * getLevelName(int _level)
      { if ((_level>=0)&&(_level<=nLevels)) return level[_level]->getName(); else return nullptr; }

    const char * getFieldName(int _level, int n, int *lN=nullptr, int *_arrNameOffset=nullptr)
      { if ((_level>=0)&&(_level<=nLevels)) return level[_level]->getFieldName(n, lN, _arrNameOffset); else return nullptr; }
    char * getElementName(int _level, int n) // caller must free() returned string!!
      { if ((_level>=0)&&(_level<=nLevels)) return level[_level]->getElementName(n); else return nullptr; }

    cVectorMeta * getLevelMetaDataPtr(int _level)
      { if ((_level>=0)&&(_level<=nLevels)) return level[_level]->getLevelMetaDataPtr(); else return nullptr; }
