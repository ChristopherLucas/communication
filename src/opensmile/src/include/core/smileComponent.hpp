...
public:
  ...
  cComponentMessage(const char *type, const char *name=nullptr) :
    sender(nullptr), smileTime(0.0), userTime1(0.0), userTime2(0.0), 
    readerTime(-1.0), msgid(-1),
    userflag1(0), userflag2(0), userflag3(0),
    custData(nullptr), custData2(nullptr), custDataSize(0), custData2Size(0),
  ...
  {
    char * custDataText = nullptr;
    if (myCustData != nullptr) {
  ...
  char * serializeToJson(int detail = 99, const char *recepient = nullptr) {
    ...
    char * custDataText = nullptr; // custDataType
    char * custData2Text = nullptr;
    if (detail >= 40) {
    ...
    if (custDataText != nullptr)
      free(custDataText);
    if (custData2Text != nullptr)
      free(custData2Text);
    if (recepientJson != nullptr)
      free(recepientJson);
    return json;
  }
};
...
#define SMILECOMPONENT_CREATE(TP) cSmileComponent * TP::create(const char*_instname) { \
                                                  ...
                                                  if (c!=nullptr) c->setComponentInfo(sconfman,scname,sdescription); \
                                                  return c; \
                                                  }
                                                  
// create for an abstract class
#define SMILECOMPONENT_CREATE_ABSTRACT(TP) cSmileComponent * TP::create(const char*_instname) { return nullptr; }
...
#define SMILECOMPONENT_REGCOMP_INIT  if (_confman == nullptr) return nullptr; \
                                     int rA = 0; \
									 sconfman = _confman;

#define SMILECOMPONENT_CREATE_CONFIGTYPE    ConfigType *ct = new ConfigType(scname); \
	                                        if (ct == nullptr) OUT_OF_MEMORY;
#define SMILECOMPONENT_INHERIT_CONFIGTYPE(configtype)  ConfigType *ct = nullptr; \
...
#define SMILECOMPONENT_MAKEINFO(TP)  if ((rA!=0)&&(ct!=nullptr)) delete ct; \
	                                 return makeInfo(sconfman, scname, sdescription, TP::create, rA)
#define SMILECOMPONENT_MAKEINFO_ABSTRACT(TP) if ((rA!=0)&&(ct!=nullptr)) delete ct; \
...
#define SMILECOMPONENT_MAKEINFO_NODMEM(TP) if ((rA!=0)&&(ct!=nullptr)) delete ct; \
	                                       return makeInfo(sconfman, scname, sdescription, TP::create, rA, 0, 1)

// make info for non-datamemory components (e.g. the cFunctionalXXXX components), e.g. components that are sub-components of other components
#define SMILECOMPONENT_MAKEINFO_NODMEM_ABSTRACT(TP) if ((rA!=0)&&(ct!=nullptr)) delete ct; \
	                                                return makeInfo(sconfman, scname, sdescription, TP::create, rA, 1, 1)
...
class  cSmileComponent {
  ...
  protected:
    ...
    virtual int myRegisterInstance(int *runMe=nullptr) { return 1; }
  ...
  public:
    ...
    virtual void setComponentEnvironment(cComponentManager *compman, int id, cSmileComponent *parent=nullptr);
    ...
    void setConfigInstanceName(const char *cfname) {
      if (cfname != nullptr) {
        if ((cfname_ != nullptr) && (cfname_ != iname_)) {
          free(cfname_);
          cfname_ = nullptr;
        }
    ...
    int registerInstance(int *runMe=nullptr) {
      if (runMe != nullptr) *runMe = runMe_;
      if (isRegistered_) return 1;
      isRegistered_ = myRegisterInstance(runMe);
      if (runMe != nullptr) runMe_ = *runMe;
