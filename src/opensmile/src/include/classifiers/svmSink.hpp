...
public:
  smileSvmModel(cSmileComponent *_parent, const char * _model) : parent(_parent), model(_model),
    classnames(nullptr), attributeNames(nullptr), binSvm(nullptr), ftSelMap(nullptr), nFtSel(0), showStatsDebug(0),
    ignoreLogitModel(0)
    {}
  ...
  int evaluate(FLOAT_DMEM *v , long N, const char ** winningClass = nullptr, FLOAT_DMEM ** probs=nullptr, FLOAT_DMEM *conf=nullptr, int winningClassMethod=SVMSINK_WINNING_CLASS_VOTE, int usePairwiseCoupling=1);
