...
class juliusResult {
  public:
    ...
    juliusResult() : numW(0), word(nullptr), phstr(nullptr), conf(nullptr), start(nullptr), end(nullptr), wordFlag(nullptr), amScore(0.0), lmScore(0.0), score(0.0), duration(0.0), userFlag(0) {}

    juliusResult( int N ) : numW(N), word(nullptr), phstr(nullptr), conf(nullptr), start(nullptr), end(nullptr), wordFlag(nullptr), amScore(0.0), lmScore(0.0), score(0.0), duration(0.0), userFlag(0) {
      if (numW > 0) {
    ...
    ~juliusResult() {
      int i;
      if (word != nullptr) {
        for (i=0; i<numW*2; i++) { if (word[i] != nullptr) free(word[i]); }
        free(word);
      }
      if (conf != nullptr) { free(conf); }
      if (wordFlag != nullptr) { free(wordFlag); }
    }
};
