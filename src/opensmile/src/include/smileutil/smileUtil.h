...
#define LOGLEVEL 2
#define SMILE_PRINT(...) Rprintf(,__VA_ARGS__)
#define SMILE_PRINTL(level, ...) { if (level <= LOGLEVEL) Rprintf(,__VA_ARGS__); }


#define SMILE_MSG(level, ...) { if (level <= LOGLEVEL) { Rprintf("MSG(%i): ",level) ; Rprintf(__VA_ARGS__); } }
#define SMILE_IMSG(level, ...) { if (level <= LOGLEVEL) { Rprintf("MSG(%i): ",level) ; Rprintf(__VA_ARGS__); } }

#define SMILE_ERR(level, ...) { if (level <= LOGLEVEL) { Rprintf("ERR(%i): ",level) ; Rprintf(__VA_ARGS__); } }
#define SMILE_IERR(level, ...) { if (level <= LOGLEVEL) { Rprintf("ERR(%i): ",level) ; Rprintf(__VA_ARGS__); } }

#define SMILE_WRN(level, ...) { if (level <= LOGLEVEL) { Rprintf("WRN(%i): ",level) ; Rprintf(__VA_ARGS__); } }
#define SMILE_IWRN(level, ...) { if (level <= LOGLEVEL) { Rprintf("WRN(%i): ",level) ; Rprintf(__VA_ARGS__); } }


#ifdef DEBUG
#define SMILE_DBG(level, ...) { if (level <= LOGLEVEL) { Rprintf("DBG(%i): ",level) ; Rprintf(__VA_ARGS__); } }
#define SMILE_IDBG(level, ...) { if (level <= LOGLEVEL) { Rprintf("DBG(%i): ",level) ; Rprintf(__VA_ARGS__); } }
#endif
...
/* free the temporal median filter workspace and return nullptr */
...
/*
  ...
  (if workspace is nullptr , the function will allocate and free the workspace internally)
*/
...
// returns: peak x of parabola, and optional (if not nullptr) the y value of the peak in *y and the steepness in *_a
...
/* Rectangular window */
 double * smileDsp_winRec(long N_);

/* Triangular window (non-zero endpoint) */
 double * smileDsp_winTri(long N_);

/* Powered triangular window (non-zero endpoint) */
 double * smileDsp_winTrP(long N_);

/* Bartlett window (triangular, zero endpoint) */
 double * smileDsp_winBar(long N_);

/* Hann window */
 double * smileDsp_winHan(long N_);

/* Hamming window */
 double * smileDsp_winHam(long N_);

/* Sine window */
 double * smileDsp_winSin(long N_);

/* Gauss window */
 double * smileDsp_winGau(long N_, double sigma);

/* Lanczos window */
 double * smileDsp_winLac(long N_);

/* Blackman window */
 double * smileDsp_winBla(long N_, double alpha0, double alpha1, double alpha2);

/* Bartlett-Hann window */
 double * smileDsp_winBaH(long N_, double alpha0, double alpha1, double alpha2);

/* Blackman-Harris window */
 double * smileDsp_winBlH(long N_, double alpha0, double alpha1, double alpha2, double alpha3);
 ...
 * *mag and *phases can be nullptr, to disable computation of either.
 ...
 Parameters:
   ...
   *coeffs : array which will hold the LPC coefficients
   ...
   gbb, gb2, gaa : work area memory,
     ...
     if these pointers are nullptr, smileDsp_calcLpcBurg will allocate
...
the memory pointed to by *h must be initialised with 0s! (at least h->bins must be nullptr...)
*/
...
/*read samples from wave file (after reading header). 
  ...
  *pcmParam points to the struct filled by smilePcm_readWaveHeader(...)
  ...
  return value : -1 eof (or filehandle==nullptr), 0 error, > 0 , num samples read
  the filehandle will automatically be closed and set to nullptr at the end of file
*/
