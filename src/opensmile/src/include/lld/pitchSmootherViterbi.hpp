...
public:
  ...
  wrIdx(0), rdIdx(0), buflen(_buflen), nStates(_nStates), frameSize(_frameSize),  prev(nullptr), convIdx(-1)
  {
  ...
  ~cSmileViterbi() {
    ...
    if (paths[0] != nullptr) free(paths[0]);
    if (paths[1] != nullptr) free(paths[1]);
    if (pathCosts != nullptr) free(pathCosts);
    if (bestPath != nullptr) free(bestPath);
    if (pathCostsNew != nullptr) free(pathCostsNew);
    if (pathCostsTemp != nullptr) free(pathCostsTemp);
  }
