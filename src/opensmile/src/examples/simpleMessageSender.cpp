...
SMILECOMPONENT_REGCOMP(cSimpleMessageSender)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ct->setField("messageRecp", "A comma separated list of message receipients (component names). If you leave this empty, no messages will be sent.", (const char *)nullptr);
    ...
    ct->setField("dataElementName", "The name of the input level data element to send periodically or base the event decisions on. If left empty, the first element will be used.", (const char *)nullptr);
    ...
    ct->setField("showCustDataAsText", "1/0 = enable/disable printing of (non nullptr) custData field as text string in debug message logs.", 0);
    ct->setField("showCustData2AsText", "1/0 = enable/disable printing of (non nullptr) custData2 field as text string in debug message logs.", 0);
...
cSimpleMessageSender::cSimpleMessageSender(const char *_name) :
  ...
  dataElementIndex(0), dataElementName(nullptr),
  messageRecp(nullptr), condFlag(false),
...
int cSimpleMessageSender::myFinaliseInstance()
{
  ...
  if (dataElementName != nullptr && dataElementIndex != -1) {
...
{
  if (enableDebugReceiver && msg != nullptr) {
...
void cSimpleMessageSender::printMessage(cComponentMessage *msg)
{
  ...
  if (showCustDataAsText && msg->custData != nullptr) {
    SMILE_PRINT("  CustData : '%s'", msg->custData);
  }
  if (showCustData2AsText && msg->custData2 != nullptr) {
...
int cSimpleMessageSender::myTick(long long t)
{
  ...
  if (vec == nullptr) return 0;  // no data available
