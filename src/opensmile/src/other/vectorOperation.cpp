...
SMILECOMPONENT_REGCOMP(cVectorOperation)
{
  ...
  ct->setField("nameBase","base of output feature name when performing n->1 mapping operations (currently 'euc' and 'sum')", (const char*)nullptr);
...
cVectorOperation::cVectorOperation(const char *_name) :
  ...
  param2(0.0),
  nameBase(nullptr),
  operationString_(nullptr),
  targFreqScaleStr(nullptr)
{
...
int cVectorOperation::setupNamesForField(int i, const char*name, long nEl)
{
  if (operation == VOP_FCONV) {
    if (nameAppend_ == nullptr || nameAppend_[0] == 0) {
    ...
    if (operationString_ != nullptr) {
      nameAppend_ = operationString_;
    }
  }
  if (nameBase != nullptr && nameBase[0] != 0) {
    name = nameBase;
  }
...
int cVectorOperation::setupNewNames(long ni)
{
  ...
  if (operation > VOP_X) {
    ...
    if (copyInputName_) {
      ...
      if (nameAppend_ != nullptr && nameAppend_[0] != 0) {
        addNameAppendField(nullptr, nameAppend_, 1, 0);
      }
    }
  } else {
