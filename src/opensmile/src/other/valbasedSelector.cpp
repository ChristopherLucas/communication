...
cValbasedSelector::cValbasedSelector(const char *_name) :
  ...
  myVec(nullptr), idx(0), invert(0), removeIdx(0), threshold(0.0), outputVal(0.0), zerovec(0), elI(0),
  valBuf_(nullptr), valBufSize_(0), valBufIdx_(0),
...
int cValbasedSelector::myTick(long long t)
{
  ...
  if (copy) {
    ...
    if (removeIdx == 1) {
      ...
      if (myVec==nullptr) myVec = new cVector(vec->N-1 , vec->type);
      int j,n=0;
      for(j=0; j<vec->N; j++) {
        if (j!=i) {
