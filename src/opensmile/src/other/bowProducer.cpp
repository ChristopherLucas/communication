...
SMILECOMPONENT_REGCOMP(cBowProducer)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ct->setField("kwList","text file with list of keywords (one word per line) to use for BoW",(const char*)nullptr);
    ...
    ct->setField("textfile","A file that contains sentences (words separated by spaces), which will be converted to BOW vectors. You can use this feature to use this component in offline mode (in online mode the text strings will be received as smile messages).",(const char *)nullptr);
    ct->setField("singleSentence","A single sentence to be converted to a BoW vector. Words must be separated by spaces.",(const char *)nullptr);
...
cBowProducer::cBowProducer(const char *_name) :
  cDataSource(_name),kwList(nullptr),count(0),numKw(0),
  keywords(nullptr), prefix(nullptr), dataFlag(0), filehandle(nullptr), 
  lineNr(0), line(nullptr), line_n(0), txtEof(0)
{
}
  ...
  if (kwList != nullptr) {
    ...
    do {
      ...
      if ((read != (size_t)-1)&&(line!=nullptr)) {
        ...
        if (len > 0) {
          if ((!kwListPrefixFilter)||(prefix == nullptr)||(!strncmp(line,prefix,MIN(strlen(prefix),strlen(line))))) {
            numKw++;
...
int cBowProducer::setupNewNames(long nEl)
{
  ...
  for (i=0; i<numKw; i++) {
    char *tmp;
    if ((kwListPrefixFilter)&&(prefix != nullptr)) {
...
int cBowProducer::myTick(long long t)
{
  ...
  */
    ...
    if ((read != -1)&&(line!=nullptr)) {
      ...
      // if EOF, close file and filehandle=nullptr;
      fclose(filehandle); filehandle=nullptr;
      if (line != nullptr) {
        free(line);
        line = nullptr;
      }
  ...
  } else if (singleSentence != nullptr) { // single sentence from commandline
