...
SMILECOMPONENT_REGCOMP(cWindower)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ...
    ct->setField("saveWindowToFile", "Save window to text file (comma separated coefficients) of given name. If nullptr (default) nothing will be saved.", (const char*)nullptr);
  )
...
void cWindower::precomputeWinFunc()
{
  ...
  switch(winFunc) {
    ...
    default: SMILE_ERR(1,"unknown window function ID (%i) !",winFunc); win=nullptr;
  }
  if ((win != nullptr)&&(squareRoot)) {
  ...
  if ((win != nullptr)&&(gain!=1.0)) { // apply gain
    for (i=0; i<frameSizeFrames; i++) {
      win[i] *= gain;
    }
  }
