...
SMILECOMPONENT_REGCOMP(cTurnDetector)
{
  ...
  //---  
    ...
    ct->setField("messageRecp","The (cWinToVecProcessor type) component(s) to send 'frameTime' messages to (use , to separate multiple recepients), leave blank (nullptr) to not send any messages. The messages will be sent at the turn end and (optionally) during the turn at fixed intervals configured by the 'msgInterval' parameter (if it is not 0).",(const char *) nullptr);
    ...
    ct->setField("eventRecp","The component(s) to send 'turnStart/turnEnd' messages to (use , to separate multiple recepients), leave blank (nullptr) to not send any messages",(const char *) nullptr);
    ct->setField("statusRecp","The component(s) to send 'turnSpeakingStatus' messages to (use , to separate multiple recepients), leave blank (nullptr) to not send any messages",(const char *) nullptr);
...
cTurnDetector::cTurnDetector(const char *_name) :
  ...
  turnTime(0.0),  
  msgInterval(0.0),
  blockAll(1), blockStatus(0),
  blockTurn(0), unblockTurnCntdn(0), 
  ...
  lastDataTime(0),
  timeoutSec(10.0), 
  debug(4), cnt1(0), cnt2(0), cnt1s(0), cnt2s(0),
  rmsIdx(-1),
  autoRmsIdx(-1),  
  autoThreshold(0),
  nmin(0), nmax(0),
  ...
  dtMeanO(0.0),  dtMeanT(0.0), dtMeanS(0.0),
  alphaM((FLOAT_DMEM)0.9999),  
  nE(0.0),  dtMeanAll(0.0),
  nTurn(0), nSil(0),
  calCnt(0),
  recFramer(nullptr),
  recComp(nullptr),
...
void cTurnDetector::connectSetWaveFrameBordersCB(SetWaveFrameBordersCB_Ptr setWaveFrameBordersCB_)
{
  setWaveFrameBordersCB = setWaveFrameBordersCB_;
}
...
int cTurnDetector::myTick(long long t)
{
  ...
  if ((isEOI() || timeout || isPaused()) && turnState) {
    ...
    SMILE_MSG(1, "turn end at EOI (%i|%i|%i|%i) at vIdx %i (tick nr %llu)!",isEOI(),timeout,isPaused(),turnState,lastVIdx - eoiMis,t);
    if (recComp!=nullptr) {
      ...
      SMILE_MSG(0, "sending turnEnd message to '%s' (due to TIMEOUT)",recComp);      
    }
    if (recFramer!=nullptr) {
      if (sendTurnFrameTimeMessageAtEnd_
        ...
        SMILE_MSG(1, "sending turnFrameTime message (turn end) to '%s' (due to TIMEOUT) (%ld - %ld)",
                  recFramer, (long)cmsg.floatData[0], (long)cmsg.floatData[1]);
        if(nullptr != setWaveFrameBordersCB)
          setWaveFrameBordersCB(getCompMan(), cmsg.floatData[0], cmsg.floatData[1]);
      } else {
        if (!sendTurnFrameTimeMessageAtEnd_) {
  ...
  if (!autoThreshold || calCnt > 200) { /* TODO: make autoThresholdCalibrationTime configurable!*/
    ...
    if (vo) {  /* if voice activity */
      ...
      if (!actState) {
        if (cnt1s<=0) { 
          if (vec->tmeta != nullptr) startSmileTimeS = vec->tmeta->smileTime; 
        }
        cnt1s++;
        if (cnt1s > 1) {
          actState = 1;
          cnt1s=0;
          cnt2s=0;
          if (statusRecp != nullptr) {
            SMILE_IMSG(debug + 1, "detected voice activity start at vIdx %i!",
                vec->tmeta->vIdx - 1);
            SMILE_MSG(1, "detected voice activity start at vIdx %i!",
                      vec->tmeta->vIdx - 1);              
            ...
            SMILE_MSG(1, "sending turnSpeakingStatus (1) message to '%s'",recComp);             
          }
        }
      }
      ...
      if (!turnState) {
        if (cnt1<=0) { 
          if (vec->tmeta != nullptr) startSmileTime = vec->tmeta->smileTime;/*offline: time !  FIX THIS PERMANENTLY...*/
        }
        cnt1++;
        if (cnt1 > nPre) {
          ...
          SMILE_MSG(1,"detected turn start at vIdx %i!",startP);          
          if (recComp!=nullptr) {
            ...
            SMILE_MSG(1,"sending turnStart message to '%s'",recComp);             
          }
        }
      }
    } else {
      ...
      cnt1s=0;
      if (cnt2 <= 0) {
        if (vec->tmeta != nullptr) endSmileTime = vec->tmeta->smileTime;/*offline: time !  FIX THIS PERMANENTLY...*/
      }
      if (actState) {
        cnt2s++;
        if (cnt2s > 5) {
          ...
          if (statusRecp != nullptr) {
            SMILE_IMSG(debug + 1, "detected voice activity end at vIdx %i!",
                vec->tmeta->vIdx - 2);
            SMILE_MSG(1,"detected voice activity end at vIdx %i!",
                      vec->tmeta->vIdx - 2);             
            cComponentMessage cmsg("turnSpeakingStatus");
            cmsg.intData[0] = 0;
            ...
            SMILE_MSG(1,"sending turnSpeakingStatus (0) message to '%s'",recComp);  
          }
        }
      }
      cnt2++;
      ...
      if (turnState) {
        if (cnt2 > nPost) {
          ...
          SMILE_MSG(1,"detected turn end at vIdx %i !",(vec->tmeta->vIdx)-nPost);
          ...
          if (recFramer!=nullptr) {
            if ((sendTurnFrameTimeMessageAtEnd_) &&
              ...
              SMILE_MSG(1,"sending turnFrameTime message (turn end) to '%s' (%ld - %ld, blag: %ld).",
                        recFramer, thisStartP, thisEnd - blag, blag);
              if(nullptr != setWaveFrameBordersCB)
                setWaveFrameBordersCB(getCompMan(), thisStartP, thisEnd - blag);
            } else {
              if (sendTurnFrameTimeMessageAtEnd_) {
          ...
          if (recComp!=nullptr) {
            ...
            SMILE_MSG(1,"sending turnEnd message to '%s'",recComp);              
          }
          forceEnd = 0;
    ...
    if (vec->tmeta->vIdx - startP > minTurnLengthFrameTimeFrames_) {
      if (turnTime > msgInterval) {
        ...
        if (recFramer!=nullptr) {
          ...
          SMILE_MSG(1,"sending turnFrameTime message (intra turn) to '%s' (%ld -> %ld)",
                    recFramer, thisStartP, thisEnd);
          if(nullptr != setWaveFrameBordersCB)
            setWaveFrameBordersCB(getCompMan(), thisStartP, thisEnd);          
        }
      } 
    }
