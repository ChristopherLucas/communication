 ...
 ***************************************************************************E*/
////////////////////////////////////////
//VERY IMPORTANT order of includes!!!
////////////////////////////////////////
...
void cNnLSTMcell::reset() 
{
  ...
  // reset outputs
  if ((cellOutputs != nullptr) && (nOutputs > 1)) {
...
const FLOAT_NN * cNnLSTMcell::forward(const FLOAT_NN *x, long *N) 
{
  ...
  if (nCells == 1) { // quick version for one cell per block
    ...
    *sc = actIG * *cNnNNcell::forward(x+2) + (*sc)*actFG;
    ...
    //XX//Rprintf("pg: %f --> cellOutput %f   actOG %f   ag: %f\n",*sc,cellOutput,actOG,transferOut->f( *sc ));
...
void cNnConnection::printInfo()
{
  ...
  for (i=0; i<nInputs; i++) {
    if (input[i] != nullptr) 
      Rprintf(" '%s' (%i); ",input[i]->getName(),input[i]->getLayerIdx());
  }
  ...
  */
  ...
  if (!bidirOut) { // "forward pass"
    ...
    for (i=0; i<nInputs; i++) {
      ...
      //XX//for (n=0; n<size; n++) { if (tmp[n] != 0.0) { Rprintf("input %i , tmp %i\n",i,n); } }
      ...
      for (j=0; j<outputSize; j++) {
        for (n=0; n<size; n++) {
          //XX//if (*wght != 0.0012) Rprintf("n  %i j %i\n",n,j);
          *outp += tmp[n]* (*(wght++));
        }
        outp++;
      }
      //XX//Rprintf("outputS   %i %i\n",outputSize,size);
    }

    // pass output to output layer
    //XX//for (n=0; n<outputSize; n++) { if (outputs[n] != 0.0) { Rprintf("FWoutput %i = %f\n",n,outputs[n]); } }
...
void cNnRnn::forward(FLOAT_NN *x, long N)
{
  ...
  for (i=1; i<=nConnections; i++) {
    if (connection[i] != nullptr) connection[i]->forward();
    else { RNN_ERR(1,"connection[%i] is nullptr",i); }
  }
}
...
{
  int i;
  Rprintf("%i net layers:\n",nLayers);
  for (i=0; i<nLayers; i++) {
    Rprintf("  %i : '%s'\n",i,layer[i]->getName());
  }
  for (i=1; i<=nConnections; i++) {
    if (connection[i] != nullptr) {
      Rprintf("--CONNECTION %i--\n",i);
...
int smileRnn_parseHiddenType(const char *n1, int layerIdx, cRnnNetFile &net, const char *filename = nullptr)
{
...
/*      if (!bidirectional) {
    ...
    SMILE_ERR(1,"failed to open rnn net file, the filename is a nullptr string.");
  }
  return 0;
}
