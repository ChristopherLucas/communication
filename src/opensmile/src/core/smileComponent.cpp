...
// register component will return nullptr, if the component could not be registered (fatal error..)
...
SMILECOMPONENT_REGCOMP(cSmileComponent)
{
  ...
  //ConfigType *ct = new ConfigType(scname); if (ct == nullptr) OUT_OF_MEMORY;
  ...
  override_(0)
{
  ...
  if (instname == nullptr) COMP_ERR("cannot create cSmileComponent with instanceName == nullptr!");
...
void cSmileComponent::setComponentEnvironment(cComponentManager *compman, int id, cSmileComponent *parent)
{
  if (compman != nullptr) {
    ...
    SMILE_IERR(3, "setting nullptr componentManager in cSmileComponent::setComponentEnvironment!");
  }
  parent_ = parent;
  mySetEnvironment();
}
...
int cSmileComponent::sendComponentMessage(const char *recepient, cComponentMessage *msg)
{
  int ret=0;
  if (compman_ != nullptr) {
    if (msg != nullptr) msg->sender = getInstName();
...
{ 
  if (compman_ != nullptr) return compman_->isAbort();
  else return 0; 
}
