...
SMILECOMPONENT_REGCOMP(cDataProcessor)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ...
    ct->setField("nameAppend", "A string suffix to append to the input field names (default: empty)", (const char*)nullptr);
...
cDataProcessor::cDataProcessor(const char *_name) :
  ...
  nInput_(0), inputStart_(0), inputName_(nullptr),  
  writer_(nullptr),
  reader_(nullptr),
  namesAreSet_(0),
  buffersize_sec_(0.0),
...
void cDataProcessor::fetchConfig()
{
  ...
  if (nameAppend_ != nullptr) { SMILE_IDBG(2,"nameAppend = '%s'",nameAppend_); }
...
int cDataProcessor::myConfigureInstance()
{
  ...
  if (c==nullptr) COMP_ERR("myConfigureInstance: Error getting reader dmLevel config! returned sDmLevelConfig = nullptr!");
...
// Adds a field with given parameters, concat base name and append string (if not nullptr).
void cDataProcessor::addNameAppendField(const char*base, const char*append, int N, int arrNameOffset)
{
  char *xx;

  if  ((append != nullptr)&&(strlen(append)>0)) {
...
void cDataProcessor::addNameAppendFieldAuto(const char*base, const char *customFixed, int N, int arrNameOffset)
{
  ...
  if  ((nameAppend_ != nullptr)&&(strlen(nameAppend_)>0)) {
    if ((customFixed != nullptr)&&(strlen(customFixed)>0)) {

      if ((copyInputName_)&&(base != nullptr)&&(strlen(base)>0)) {
...
{
  int more = 0;
  inputStart_ = findField(namePartial, fullName, &nInput_, &inputName_, nEl, &more, nullptr);
  return inputStart_;
...
long cDataProcessor::findField(const char *namePartial, int fullName, long *N, const char **fieldName, long nEl, int *more, int *fieldIdx)
{
  ...
  if (fieldIdx != nullptr) *fieldIdx = idx;
  if (N != nullptr) *N = nInput;
  if (fieldName != nullptr) *fieldName = inputName;
  return inputStart;
}
...
// stores nInput elements in **_in, the memory is allocated if necessary (i.e. *_in == nullptr)
int cDataProcessor::getInputFieldData(const FLOAT_DMEM *src, long Nsrc, FLOAT_DMEM **in)
{
  if (nInput_ <= 0) return 0;
  if (in != nullptr) {
    if (*in == nullptr) *in = (FLOAT_DMEM*)malloc(sizeof(FLOAT_DMEM)*nInput_);
    long i;
    for (i=0; i<nInput_; i++) {
...
int cDataProcessor::setupNamesForField(int i, const char*name, long nEl)
{
  if (copyInputName_) {
    ...
    addNameAppendField( nullptr, nameAppend_, nEl );
  }
  return nEl;
}
...
{
  const FrameMetaInfo * fmeta = reader_->getFrameMetaInfo();
  if ((fmeta != nullptr) && (sourceFidx < fmeta->N)) {
