...
//    ct->setField("frameBorderList","array list of frame borders (in seconds), if frameMode==list",(const char*)nullptr, ARRAY_TYPE);
//    ct->setField("frameList","array list of frame start/end times (in seconds) (specifiy as: '0.3-1.7 ; 0.9-2.1', for example), if frameMode==list (use either this OR frameBorderList)",(const char*)nullptr, ARRAY_TYPE);
  )
...
cWinToVecProcessor::cWinToVecProcessor(const char *_name) :
  ...
  ivSec(nullptr), ivFrames(nullptr),  
  frameIdx(0),
  matBufN(0), matBufNalloc(0),  
  lastText(nullptr), lastCustom(nullptr),
  matBuf(nullptr),
  tmpRow(nullptr),
  tmpVec(nullptr),
  tmpFrameF(nullptr),
...
void cWinToVecProcessor::fetchConfig()
{
  ...
  if (frameMode == FRAMEMODE_LIST) {
    ...
    if (isSet("frameListFile")) { // prefer file over config file list
      ...
      if (flf != nullptr) { // open file and read frame list (only 1st line!)
        FILE *f = fopen(flf,"r");
        size_t n;
        smile_getline(&fl,&n,f);
        fclose(f);
      } else { // display error message, shouldn't happen
        SMILE_IERR(1,"cannot open frameListFile 'nullptr'");
        COMP_ERR("aborting");
      }
    } else {
      if (isSet("frameList")) {
        ...
        if (_fl != nullptr) {
          ...
          SMILE_IERR(1,"frameList string is 'nullptr'");
          COMP_ERR("aborting");
        }
      } else {
    ...
    if (fl != nullptr) { // if frameList has been loaded
      //printf("Loaded: %s \n",fl);
...
int cWinToVecProcessor::dataProcessorCustomFinalise() 
{
  ...
  if (!wholeMatrixMode) {
    ...
    if (tmpFrameF==nullptr) tmpFrameF=(FLOAT_DMEM*)calloc(1,sizeof(FLOAT_DMEM)*Mult);
    if (tmpFrameI==nullptr) tmpFrameI=(INT_DMEM*)calloc(1,sizeof(INT_DMEM)*Mult);
  }
  return 1;
}
...
int cWinToVecProcessor::getNextFrameData(double *start, double *end, int *flag, int *ID)
{
  if ((start!=nullptr)&&(end!=nullptr)) {
    ...
    if (nQ > 0) {
      *start = Qstart[0];
      *end = Qend[0];
      if (flag != nullptr) *flag = Qflag[0];
      if (ID != nullptr) *ID = QID[0];
      int i;
      for (i=0; i<nQ-1; i++) {
        Qstart[i] = Qstart[i+1];
        Qend[i] = Qend[i+1];
...
int cWinToVecProcessor::myTick(long long t)
{
  ...
  if (frameMode == FRAMEMODE_META) { 
    ...
    if ((vec->tmeta->metadata.iData[1] != 1234)||(vec->tmeta->metadata.text == nullptr)) {
      ...
      if ((lastText!=nullptr)&&(strcmp(lastText,vec->tmeta->metadata.text))) {
        ...
        if (lastText == nullptr) {
          ...
          if (lastCustom != nullptr) free(lastCustom);
...
//       }
  ...
  if ((mat == nullptr) && (!(isEOI()&&EOIlevelIsMatch()))) { 
  ...
  if (mat==nullptr) {
    ...
    if (c!=nullptr) type = c->type;
    else return 0;
  } else { type=mat->type; }

  if (tmpVec==nullptr) tmpVec = new cVector(No,type);
  
  if (frameMode == FRAMEMODE_META) { 
    // set tmeta correctly in tmpVec...      
    if (tmpVec->tmeta->metadata.text != nullptr) free(tmpVec->tmeta->metadata.text);
    tmpVec->tmeta->metadata.text = strdup(lastText);
    if (tmpVec->tmeta->metadata.custom != nullptr) free(tmpVec->tmeta->metadata.custom);
    ...
    // next...
    if (lastText != nullptr) free(lastText);
    lastText = strdup(vec->tmeta->metadata.text);
    if (lastCustom != nullptr) free(lastCustom);
    lastCustom = strdup((char*)(vec->tmeta->metadata.custom));
  }

  //if ((tmpRow==nullptr)&&(mat!=nullptr)) tmpRow = mat->getRow(0);
...
{
  if (tmpFrameF!=nullptr) free(tmpFrameF);
  if (tmpFrameI!=nullptr) free(tmpFrameI);
  if (ivSec != nullptr) free(ivSec);
  if (ivFrames != nullptr) free(ivFrames);
  if (tmpVec!=nullptr) delete tmpVec;
  if (tmpRow != nullptr) delete tmpRow;
  if (matBuf != nullptr) delete matBuf;
  if (lastText != nullptr) free(lastText);
  if (lastCustom != nullptr) free(lastCustom);
}
