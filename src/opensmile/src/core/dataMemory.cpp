...
SMILECOMPONENT_REGCOMP(cDataMemory)
{
  ...
  if (dml == nullptr) OUT_OF_MEMORY;
  dml->setField("name", "The name of this data memory level, must be unique within one data memory instance.", (char*)nullptr);
...
long FrameMetaInfo::elementToFieldIdx(long _element, long *_arrIdx) const
{
  ...
  for(i=0; i<N; i++) {
    lN += field[i].N;
    if (lN > _element) {
      if (_arrIdx != nullptr) *_arrIdx = field[i].N - (lN - _element);
      return i;
    }
  }
  if (_arrIdx != nullptr) *_arrIdx = 0;
  return -1;
}
...
{
  if (_fieldName == nullptr) return -1;

  // check for [] in _fieldName => array field
  long ai = 0;
  char *fn = strdup(_fieldName);
  char *a  = strchr(fn,'[');
  if (a != nullptr) {  // yes, array
    a[0] = 0;
    a++;
    char *b = strchr(fn,']');
    if (b == nullptr) { COMP_ERR("findField: invalid array field name '%s', expected ']' at the end!",_fieldName); }
    b[0] = 0;
    char *eptr = nullptr;
    ai = strtol(a,&eptr,10);
    if ((ai==0)&&(eptr == a)) { COMP_ERR("findField: error parsing array index in name '%s', index is not a number!",_fieldName); }
  } 
  if (arrIdx != nullptr) *arrIdx = ai;

  int i;
  int idx = -1;
  int _start = 0;
  if ((more != nullptr)&&(*more>0)) { _start = *more; *more = 0; }
  int foundFlag = 0;
  for (i=_start; i<N; i++) {
    int myAi;
    if (!foundFlag) {
      if (a != nullptr) // array name
    ...
    if (!strcmp(fn,field[i].name)) {
      if (!foundFlag) {
        ...
        if (myAi < field[i].N) {
          ...
          if (arrIdx != nullptr) *arrIdx = myAi; // update *arrIdx with correct index (real index in data structure, not 'named' index)
...
{
  if (_fieldNamePart == nullptr) return -1;
  ...
  if (a != nullptr) {  // yes, array
    ...
    a++;
    char *b = strchr(fn,']');
    if (b == nullptr) { COMP_ERR("findField: invalid array field name part '%s', expected ']' at the end!",_fieldNamePart); }
    b[0] = 0;
    char *eptr = nullptr;
    ai = strtol(a,&eptr,10);
...
cVector::cVector(int lN, int _type) :
  N(0), tmetaArr(0), tmeta(nullptr), tmetaAlien(0), fmeta(nullptr), dataF(nullptr), dataI(nullptr),  ntmp(nullptr)  
{
  if (lN>0) {
    switch (_type) {
      case DMEM_FLOAT:
        dataF = (FLOAT_DMEM*)calloc(1,sizeof(FLOAT_DMEM)*lN);
        if (dataF==nullptr) OUT_OF_MEMORY;
        break;
      case DMEM_INT:
        dataI = (INT_DMEM*)calloc(1,sizeof(INT_DMEM)*lN);
        if (dataI==nullptr) OUT_OF_MEMORY;
        break;
      default:
    ...
    if (tmeta == nullptr) OUT_OF_MEMORY;
  }
}

const char *cVector::name(int n, int *lN)
{
  if ((fmeta!=nullptr)&&(fmeta->field!=nullptr)) {
    if (ntmp != nullptr) free(ntmp);
    int llN=-1;
    const char *t = fmeta->getName(n,&llN);
    if (lN!=nullptr) *lN = llN;
    if (llN>=0) {
...
cVector::~cVector() {
  if (dataF!=nullptr) free(dataF);
  if (dataI!=nullptr) free(dataI);
...
const char * cDataMemoryLevel::getFieldName(int n, int *lN, int *_arrNameOffset)
{
  ...
  if ( (n>=0)&&(n<lcfg.N) ) {
    if (lN!=nullptr) *lN = fmeta.field[n].N;
	if (_arrNameOffset!=nullptr) *_arrNameOffset = fmeta.field[n].arrNameOffset;
...
int cDataMemoryLevel::setFieldInfo(int i, int _dataType, void *_info, long _infoSize)
{
  if ((i>=0)&&(i < fmeta.N)) {
    ...
    if (_info != nullptr) {
      if (fmeta.field[i].info != nullptr) free(fmeta.field[i].info);
...
int cDataMemoryLevel::addField(const char *_name, int lN, int arrNameOffset)
{
  ...
  if (fmeta.N >= fmetaNalloc) { //realloc:
    FieldMetaInfo *f = FieldMetaInfo[fmeta.N+LOOKAHEAD_ALLOC];
    // TODO: fix this
...
int cDataMemoryLevel::finaliseLevel()   // finalize config and allocate data memory
{
  ...
  if (data==nullptr) COMP_ERR("cannot allocate level of unknown type %i or out of memory!",lcfg.type);
...
int cDataMemoryLevel::setFrame(long vIdx, const cVector *vec, int special)  // id must already be resolved...!
{
  ...
  if (vec == nullptr) {
    SMILE_ERR(3,"cannot set frame in dataMemory from a nullptr cVector object!");
    return 0;
  }
...
int cDataMemoryLevel::setMatrix(long vIdx, const cMatrix *mat, int special)  // id must already be resolved...!
{
  ...
  if (mat == nullptr) {
    SMILE_ERR(3,"cannot set frame in dataMemory from a nullptr cMatrix object!");
    return 0;
  }
...
//    for vector: return 0'ed frame instead of nullptr pointer
//                return last available frame (beginning / end)
//    for matrix: return nullptr pointer if complete matrix is out of range
...
//****************
  ...
  if (rIdx>=0) {
    ...
    vec->fmeta = &(fmeta);
    if (result!=nullptr) *result=DMRES_OK;
  } else {
...
void cDataMemory::_addLevel()
{
  ...
  if (nLevels>=nLevelsAlloc) {
    ...
    // initialize newly allocated memory with nullptr
    int i;
    for (i=nLevels; i<lN; i++) l[i] = nullptr;
    level = l;
    nLevelsAlloc=lN;
  }
}
...
int cDataMemory::registerLevel(cDataMemoryLevel *l)
{
  if (l!=nullptr) {
    ...
    level[nLevels] = l;
    return nLevels;
  } else { SMILE_WRN(1,"attempt to register nullptr level with dataMemory!"); return -1; }
}
...
void cDataMemory::registerWriteRequest(const char *lvl, const char *componentInstName)
{
  ...
  // find existing request
  int idx = wrq.findRequest(lvl, nullptr);
  ...
  if (idx) {
    ...
    if ((rq!=nullptr)&&(!strcmp(rq->instanceName,componentInstName))) {
...
int cDataMemory::myRegisterInstance(int *runMe)
{
  ...
  for (i=0; i<rrq.getNEl(); i++) {
    ...
    if (x!=nullptr) {
      int idx = wrq.findRequest(x->levelName, nullptr);
      if (!idx) {
...
cDataMemory::~cDataMemory() {
  ...
  if (level != nullptr) {
    for (i=0; i<nLevelsAlloc; i++) {
      if(level[i] != nullptr) delete level[i];
    }
    free(level);
  }
}
...
#ifdef DM_DEBUG_LOGGER
  long i;
  Rprintf("xxDMemLOGxx:: level:%s dir:%s cnt:%i vIdx0:%i vIdx:%i rIdx:%i nT:%i special:%i EOI:%i curR:%i curW:%i :: nReaders:%i ",name,dir,cnt,vIdx0,vIdx,rIdx,nT,special,EOI,curR,curW,nReaders);
  for (i=0; i<nReaders; i++) {
    Rprintf("curRr%i:%i ",i,curRr[i]);
  }
  Rprintf("::: DATA(%i) ::: ",vec->N);
  for (i=0; i<vec->N; i++) {
    Rprintf("f%i:%i ",i,vec->dataF[i]);
  }
  Rprintf("::END\n");
#endif
}
...
void datamemoryLogger(const char *name, long vIdx0, long vIdx, long rIdx, long nT, int special, long curR, long curW, long EOI, int nReaders, long *curRr, cMatrix *mat)
{
  ...
  Rprintf("::: DATA[%ix%i] ::: ",vec->N,vec->nT); // row x column
  for (j=0; j<vec->nT; j++) {
    for (i=0; i<vec->N; i++) {
      Rprintf("f%i,%i:%i ",i,j,vec->dataF[i+j*vec->N]);  // row, column
    }
    if (j<vec->nT-1) Rprintf(":: ");
  }
  Rprintf("::END\n");
#endif
}
