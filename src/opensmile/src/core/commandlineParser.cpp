...
cCommandlineParser::~cCommandlineParser()
{
  ...
  if (opt != nullptr) {
    for (i=0; i<N; i++) {
      if (opt[i].description != nullptr) free(opt[i].description);
      if (opt[i].dfltStr != nullptr) free(opt[i].dfltStr);
    }
    free(opt);
  }
}
...
{
  if ((abbr == 'h')||((name != nullptr)&&(name[0]=='h')&&(name[1]==0))) {
    COMP_ERR("option -h is reserved for show usage internally! please chose another name in your code! sorry..");
  }
  int n = getWrIdx();
  if (name == nullptr) COMP_ERR("addOpt: cannot add commandlineParser option with name==nullptr!");
          ...
          if (i+1 < argc) {
            ...
            switch (opt[n].type) {
              ...
              case CMDOPT_INT:
                //  parse value:
                errno=0;
                v = strtol(argv[i+1],nullptr,10); // values base 10... TODO: support hex
              ...
              case CMDOPT_STR:
                if (optCand) {  // bad situation... look for option
                  ...
                  if (tmp >= 0) { // next option
                    ...
                    i++; // next parameter of commandline
                    if (opt[n].dfltStr != nullptr) free(opt[n].dfltStr);
