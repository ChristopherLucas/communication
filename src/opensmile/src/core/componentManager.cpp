...
{
  if(nullptr == rcpp_wave_header || 
     0 == rcpp_audio_features_buffer.n_rows)
  {
    arma::mat addMat(features_row);
    rcpp_audio_features_buffer.insert_rows(rcpp_audio_features_buffer.n_rows, addMat);
    rcpp_audio_timestamps_buffer.insert_cols(rcpp_audio_timestamps_buffer.n_cols, timeFrame);    
  }
  else
  {
    if(nullptr == rcpp_audio_timestamps)
    {
      SMILE_MSG(1,"cComponentManager::setWaveFeaturesCB : frameStep = %lf",frameStep);
      SMILE_MSG(1,"cComponentManager::setWaveFeaturesCB : rcpp_wave_header->sampleRate = %d",rcpp_wave_header->sampleRate);
      SMILE_MSG(1,"cComponentManager::setWaveFeaturesCB : rcpp_wave_header->nBlocks = %d",rcpp_wave_header->nBlocks);
      SMILE_MSG(1,"cComponentManager::setWaveFeaturesCB : rcpp_audio_features_buffer.n_cols = %d",rcpp_audio_features_buffer.n_cols);      
      if(frameStep < 0.0000000001)
        SMILE_ERR(0,"cComponentManager::setWaveFeaturesCB : invalid frameStep = 0.");  
      int timestamps_count =  (int)(rcpp_wave_header->nBlocks / (rcpp_wave_header->sampleRate * frameStep));
      SMILE_MSG(1,"cComponentManager::setWaveFeaturesCB : timestamps_count = %d",timestamps_count);       
      rcpp_audio_timestamps = new arma::rowvec(timestamps_count);
      rcpp_audio_features = new arma::mat(timestamps_count, rcpp_audio_features_buffer.n_cols);
      for(int iRow=0; iRow<rcpp_audio_features_buffer.n_rows; iRow++ )
      {
        rcpp_audio_timestamps->operator()(iRow) = rcpp_audio_timestamps_buffer(iRow);
        for(int iCol=0; iCol<rcpp_audio_features_buffer.n_cols; iCol++)
        {
          rcpp_audio_features->operator()(iRow, iCol) = rcpp_audio_features_buffer(iRow, iCol);
        }
      }
      currentRow = rcpp_audio_features_buffer.n_rows;
    }
    
    for(int iFeat=0; iFeat<features_row.n_cols; iFeat++)
    {
      rcpp_audio_features->operator()(currentRow, iFeat) = features_row(iFeat);
    }
    rcpp_audio_timestamps->operator()(currentRow) = timeFrame(0);
    currentRow++;
  }
}

void cComponentManager::setWaveFrameBordersCB(const double &frameStart,  const double &frameEnd)
{
  SMILE_MSG(4,"cComponentManager::setWaveFrameBordersCB : frameStart = %lf, frameEnd = %lf",frameStart, frameEnd);
  SMILE_MSG(4,"cComponentManager::setWaveFrameBordersCB : rcpp_audio_start_frames.n_cols = %d", rcpp_audio_start_frames.n_cols);
  arma::rowvec time_start(1);
  arma::rowvec time_end(1);
  time_start(0) = frameStart;
  time_end(0) = frameEnd;  
  rcpp_audio_start_frames.insert_cols(rcpp_audio_start_frames.n_cols, time_start);    
  rcpp_audio_end_frames.insert_cols(rcpp_audio_end_frames.n_cols, time_end);
}


void cComponentManager::setWaveHeaderCB(const sWaveParameters &rcpp_header_)
{
  rcpp_wave_header = new sWaveParameters(rcpp_header_);
}


bool cComponentManager::getFeatures( arma::mat & rcpp_audio_features_out,
                                     arma::rowvec & rcpp_audio_timestamps_out,
                                     sWaveParameters & rcpp_wave_header_out)
{
  if(nullptr == rcpp_audio_features 
       ||
    nullptr == rcpp_audio_timestamps
       ||
    nullptr == rcpp_wave_header)
    return false;
...
void cComponentManager::registerType(cConfigManager *_confman) {
  ...
  if (_confman != nullptr) {
    ...
    if (comp == nullptr) OUT_OF_MEMORY;
    comp->setField( "type", "name of component type to create an instance of", (char*)nullptr);
    comp->setField( "configInstance", "config instance to connect to component instance (UNTESTED?)", (char*)nullptr);
    ...
    if (complist == nullptr) OUT_OF_MEMORY;
    ...
    SMILE_ERR(1,"cannot register component manager config type! _confman is nullptr in registerType()!");
  }
}
...
const char * cComponentManager::getComponentType(int i, int filter, int *isAbstract, int *isNoDmem) // get name of component Type "i". abstract=1 = include abstract types (if abstract = 0 , then nullptr will be returned if the type is abstract), or you may use the isAbstract flag, which is set by this function
{
  if ( (i>=0)&&(i<nCompTs) ) {
    if (isAbstract != nullptr) *isAbstract = compTs[i].abstract;
    if (isNoDmem != nullptr) *isNoDmem = compTs[i].noDmem;
    if (filter == 0) { // filter = 0: return all components
      ...
      if (filter == 1) {  // filter=1 : return non-abstract components
        if (compTs[i].abstract) return nullptr;
        ...
        if ((compTs[i].abstract)||(compTs[i].noDmem)) return nullptr;
        else return compTs[i].componentName;
      }
    }
  } else {
...
const char * cComponentManager::getComponentDescr(int i) // get name of component Type "i". abstract=1 = include abstract types (if abstract = 0 , then nullptr will be returned if the type is abstract), or you may use the isAbstract flag, which is set by this function
{
  if ( (i>=0)&&(i<nCompTs) ) {
...
int cComponentManager::loadPlugin(const char * path, const char * fname)
{
  ...
  if ( ((error = dlerror()) != nullptr) || (regFn == nullptr)) {
  ...
  if (nPluginHandles >= nPluginHandlesAlloc) {
    ...
    if (handlelist == nullptr) OUT_OF_MEMORY;
    if (regFnlist == nullptr) OUT_OF_MEMORY;
...
int cComponentManager::registerPlugins()
{
...
#endif
  strncat_s (szDir, MAX_PATH, "\\*", MAX_PATH);
  ...
  do {
    if (!(ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
      if (checkPluginFilename(ffd.cFileName)) {
        ...
        //#else
        //if (loadPlugin(nullptr, ffd.cFileName)) nPlugins++;
        //#endif
      }
    }
  }
  ...
  DIR *dp;
  struct dirent *dirp;
  if((dp  = opendir(pluginpath)) == nullptr) {
...
cComponentManager::cComponentManager( cConfigManager *_confman,
                                      cComponentManager::RcppModeWork rccpMode,
                                      const registerFunction _clist[]) :
printPlugins(1),
handlelist(nullptr),regFnlist(nullptr),
rccpMode(rccpMode), confman(_confman),
printLevelStats(0), profiling(0),
nCompTs(0), nCompTsAlloc(0),
compTs(nullptr), ready(0),
isConfigured(0), isFinalised(0), 
EOI(0),
EOIcondition(0),
nComponents(0),
nComponentsAlloc(0),
lastComponent(0),
componentThreadId(nullptr),
messageCounter(0), 
...
abortRequest(0)
{
  ...
  if (confman == nullptr) COMP_ERR("cannot create component manager with _confman == nullptr!");
  ...
  if (component == nullptr) OUT_OF_MEMORY;
  if (componentInstTs == nullptr) OUT_OF_MEMORY;
  if (componentThreadId == nullptr) OUT_OF_MEMORY;
  ...
  if ((insts!=nullptr)&&(N_>0)) {
    ...
    for (i=0; i<N_; i++) {
      ...
      if (k!=nullptr) {
        ...
        // check config:
        if (tp == nullptr) CONF_INVALID_ERR("%s.instance[%s].type is missing!",CM_CONF_INST,k);
        if (ci == nullptr) ci = k;
...
int cComponentManager::getNextComponentId()
{
  if (lastComponent >= nComponentsAlloc) {  // reallocate a larger component array
    ...
    if (tmpn == nullptr) OUT_OF_MEMORY;
    if (tmpi == nullptr) OUT_OF_MEMORY;
...
int cComponentManager::addComponent(const char *_instname, const char *_type, const char *_ci, int _threadId /*, int threadPrio */)
{
  ...
  if (t >= 0) {
    ...
    if (c==nullptr) COMP_ERR("failed creating component '%s' (type: '%s')",_instname,_type);
    if (_ci != nullptr) c->setConfigInstanceName(_ci);
...
cSmileComponent * cComponentManager::createComponent(const char *_instname, int n)
{
  if ((n>=0)&&(n<nCompTs)) {
    ...
    if (c==nullptr) OUT_OF_MEMORY;
    c->setComponentEnvironment(this, -1, nullptr); // set component manager reference and the component ID, used by componentManager
    return c;
  } else {
...
int cComponentManager::registerComponentInstance(cSmileComponent * _component, const char *_typename, int _threadId /*, int threadPrio */)
{
  if (_component == nullptr) {
    SMILE_ERR(1,"registerComponentInstance: cannot register nullptr component instance!");
    return -1;
  }
  int id = getNextComponentId();
  if (id >= 0) {
    _component->setComponentEnvironment(this, id, nullptr);
    //_component->setId(id);
    component[id] = _component;
    //register callbacks
    {
      {
        cRcppDataSink *rcppDataSink = dynamic_cast<cRcppDataSink *>(component[id]);
        if (nullptr != rcppDataSink &&
            RccpWavFiles == rccpMode) {
          rcppDataSink->connectSetWaveFeaturesCB(cComponentManager::staticSetWaveFeaturesCB);
        }
      }
      {
        cWaveSource *waveSource = dynamic_cast<cWaveSource *>(component[id]);
        if (nullptr != waveSource &&
            RccpWavFiles == rccpMode) {
          waveSource->connectSetWaveHeaderCB(cComponentManager::staticSetWaveHeaderCB);
        }
      }
      {
        cTurnDetector *turnDetector = dynamic_cast<cTurnDetector *>(component[id]);
        if (nullptr != turnDetector &&
            RccpWavFiles == rccpMode) {
          turnDetector->connectSetWaveFrameBordersCB(cComponentManager::staticSetWaveFrameBordersCB);
        }
      }      
      {
        cWinToVecProcessor *winToVecProcessor = dynamic_cast<cWinToVecProcessor *>(component[id]);
        if (nullptr != winToVecProcessor &&
            RccpWavFiles == rccpMode) {
          SMILE_MSG(0, "cComponentManager::registerComponentInstance frameStep=%lf", frameStep);
          frameStep = winToVecProcessor->getFrameStep();
        }
      }      
    }
...
{
  int i;
  if (_compname == nullptr) return -1;
  for (i=0; i<lastComponent; i++) {
    if ((component[i] != nullptr)&&(!strcmp(_compname,component[i]->getInstName()))) { return i; }
  }
  return -1;
}
  ...
  for (i=0; i<=lastComponent; i++) {
    if (component[i] != nullptr) {
      if (((threadId == -1)||(threadId == componentThreadId[i]))&&(componentThreadId[i]!=-2)) {
        nRunnable++;
        //Rprintf("~~~~> 'ticking' component '%s' (idx %i)\n",component[i]->getInstName(),i); 
...
{
  SMILE_DBG(0,"start threadRunner");
  sThreadData *data = (sThreadData*)_data;
  if ((data != nullptr)&&(data->obj!=nullptr)) {
    SMILE_DBG(0,"start threadRunner: (data != nullptr)&&(data->obj!=nullptr)");       
...
{
  SMILE_DBG(0,"start threadRunnerControl");  
  sThreadData *data = (sThreadData*)_data;
  if ((data != nullptr)&&(data->obj!=nullptr)) {
    SMILE_DBG(0,"start threadRunnerControl: (data != nullptr)&&(data->obj!=nullptr)");     
...
void cComponentManager::controlLoopA()
{
  ...
  do {
    ...
    if (probeFlag) {
      //      Rprintf("probe end  cr=%i nP %i nA %i\n",compRunFlag,nProbe,nActive);
      if (compRunFlag == 0) {
        // final exit!
        run = 0;
      } else {
    ...
    smileMutexUnlock(abortMtx);
    //    Rprintf("cr stat nW %i nP %i ab %i run %i pf %i\n",nWaiting,nProbe,abortRequest,run,probeFlag); 
...
long long cComponentManager::tickLoopA(long long maxtick, int threadId, sThreadData *_data)
{
  ...
  do {
    ...
    // if runFlag == 0 here...
    runFlag[threadId] = 0;
    tickNr++; //Rprintf( "tick %i eol %i eoi %i\n",tickNr,endOfLoop,EOI); 
  ...
  */
  ...
  do {
    ...
    if (nRun==0) {
      ...
      //          Rprintf("wc? %i nW %i nA %i\n",threadId,nWaiting,nActive); 
      ...
      if (nWaiting+nWaiting2 < nActive-1) {
        ...
        nRun0cnt = 0;
        //Rprintf("entering wait cond %i  (T=%i) nW %i nA %i\n",threadId,tickNr,nWaiting,nActive); 
        //smileCondBroadcastRaw(syncCond);
        //Rprintf("w1 state %i  (T=%i) nW %i nA %i nW2 %i\n",threadId,tickNr,nWaiting,nActive,nWaiting2); 
        if (nWaiting + nWaiting2 >= nActive) { smileCondBroadcastRaw(syncCond); }
        smileCondWaitWMtx(syncCond,syncCondMtx);
        //Rprintf("w1 state end %i  (T=%i) nW %i nA %i nW2 %i\n",threadId,tickNr,nWaiting,nActive,nWaiting2); 
        //Rprintf("exiting wait cond %i\n",threadId); 
        //nWaiting--;
        nRun=1;
        ...
        Rprintf("w2 state %i  (T=%i) nW %i nA %i nW2 %i\n",threadId,tickNr,nWaiting,nActive,nWaiting2); 
        //nWaiting2++;
        ...
        //Rprintf("w2 state end %i  (T=%i) nW %i nA %i nW2 %i\n",threadId,tickNr,nWaiting,nActive,nWaiting2); 

        //nRun0cnt++;

        //if (nRun0cnt <= 1) { nRun = 1; 
        //Rprintf("nRun0cnt %i = %i\n",threadId,nRun0cnt); 
        //nWaiting++;
        //           nWaiting++;
        //        smileCondBroadcastRaw(syncCond);
        //smileCondWaitWMtx(syncCond,syncCondMtx);
  ...
  long long cComponentManager::runMultiThreaded(long long maxtick)
{
  ...
  if (nThreads == 1) {
    ...
    int initRes = pthread_attr_init(&tattr);
    if(0 != initRes)
      SMILE_ERR(0,"error pthread_attr_init - %i",initRes);
    int errorCode = pthread_create(&controlThr, &tattr, threadRunnerControl, &control);
    if(0 != errorCode)
      SMILE_ERR(0,"error creating controller thread!! error - %i",errorCode);
    int destroyRes = pthread_attr_destroy(&tattr);
    if (0 != destroyRes)
      SMILE_ERR(0, "error pthread_attr_destroy - %i",destroyRes);
#else    
...
#if defined(HAVE_PTHREAD) 
      pthread_attr_t tattrThread;
      int initResThread = pthread_attr_init(&tattrThread);
      if(0 != initResThread)
        SMILE_ERR(0,"error during creating thread with threadId %i!! pthread_attr_init - %i", i ,initResThread);
      int errorCodeThread = pthread_create(&threadHandles[i], &tattrThread, threadRunner, &(threadData[i]));
      if(0 != errorCodeThread)
        SMILE_ERR(0,"error creating thread with threadId %i!! error - %i",i,errorCode);
      errorThread = (0 != errorCodeThread);
      int destroyResThread = pthread_attr_destroy(&tattrThread);
      if (0 != destroyResThread)
        SMILE_ERR(0, "error during creating thread with threadId %i!! pthread_attr_destroy - %i", i, destroyResThread);      
#else
      errorThread = !(int)smileThreadCreate(threadHandles[i], threadRunner, &(threadData[i]));
      if (errorThread) 
    ...
    if (runFlag != nullptr) free(runFlag);
    if (threadData != nullptr) free(threadData);
    if (threadHandles != nullptr) free(threadHandles);
...
cComponentManager::~cComponentManager()
{
  ...
  if ((handlelist != nullptr)&&(nPluginHandles>0)) {
