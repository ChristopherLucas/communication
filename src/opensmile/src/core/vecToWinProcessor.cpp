...
SMILECOMPONENT_REGCOMP(cVecToWinProcessor)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ...
    ct->setField("windowA","Name of cWindower component applied before transforming into the spectral domain. Leave empty to use constant window (=1).",(const char*)nullptr);
    ct->setField("windowB","Name of cWindower component applied after transforming back into the time domain. Leave empty to use constant window (=1).",(const char*)nullptr);
...
cVecToWinProcessor::cVecToWinProcessor(const char *_name) :
  ...
  ptrWinA(nullptr), ptrWinB(nullptr),
  hasOverlap(0),
  ola(nullptr),
  mat(nullptr)
{
  ...
  if (_ptrWin != nullptr) {
    ...
    if (c->frameSizeFrames != n) {
      if (n > c->frameSizeFrames) {
        ...
        if (didAlloc != nullptr) *didAlloc = 1;
      } else {
...
void cVecToWinProcessor::computeOlaNorm(long n, int idx)
{
  ...
  if (!normaliseAdd) {  // if we only want to scale with window B (=A, when useWinAasWinB is active)
    ...
    for (j=(I/2+1)*olaS; j<(I/2+1)*olaS+n; j++) {
      if (tmp[j] > 0.0) {
        ola[idx].norm[a] = gain * 0.99 * ( 1.0/tmp[j] ); // if we should use winA as winB (winB nullptr, and option set), then we must multiply winB into norm here...
...
int cVecToWinProcessor::dataProcessorCustomFinalise() 
{
  ...
  if (normaliseAdd && (ptrWinA != nullptr || ptrWinB != nullptr)) {
    if ( (ptrWinA == nullptr || ptrWinA->isFinalised()) && (ptrWinB == nullptr || ptrWinB->isFinalised()) ) {
...
int cVecToWinProcessor::myTick(long long t)
{
  ...
  */
  ...
  if (vec==nullptr) { return 0; } // currently no data available

  if (mat == nullptr) mat = new cMatrix(Nfi, inputPeriodS, DMEM_FLOAT);
...
cVecToWinProcessor::~cVecToWinProcessor()
{
  ...
  if (ola != nullptr) {
    long i;
    for (i=0; i<Nfi; i++) {
      if (ola[i].norm != nullptr) {
        free(ola[i].norm);
      }
      if (ola[i].buffer != nullptr) {
        free(ola[i].buffer);
      }
    }
    free(ola);
