...
void cDataWriter::fetchConfig() {
  ...
  if (dmLevel == nullptr) COMP_ERR("fetchConfig: getStr(dmLevel) returned nullptr! missing option in config file?");
}
...
{
  const sDmLevelConfig *c = dm->getLevelConfig(level);
  if ((c!=nullptr)&&(c->fmeta != nullptr)) return c->fmeta->N;
  else return 0;
}
