...
SMILECOMPONENT_REGCOMP(cVectorTransform)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ...
    ct->setField("initFile","The file to load the (initial) transformation data from (see vectorTransform.cpp for documentation of the file format)",(const char*)nullptr);
    ct->setField("saveFile","The file to save transformation data to (see vectorTransform.cpp for documentation of the file format)",(const char*)nullptr);
...
cVectorTransform::cVectorTransform(const char *_name) :
  ...
  initFile(nullptr), saveFile(nullptr),
  err(0), flushed(0),
  zeroThIsLast(0),
  skipNfirst(10),
  fixedBuffer(0.0),
  turnStartMessage(nullptr), turnEndMessage(nullptr),  
  updateMethod(0), mode(0),   
  alpha(0.0), weight(0.0),  
  fixedBufferFrames(0),
  nFrames(0),
  buffer(nullptr), bufferNframes(nullptr),
  nAvgBuffer(0), 
  wPtr(0), rPtr(0),
  turnOnlyNormalise(0),  
  turnOnlyOutput(0),
  ...
  resetOnTurn(0),
  turnOnlyUpdate(0),
  isTurn(0), resetMeans(0)   
{
  transform.userData = nullptr;
  transform.vectors = nullptr;
  /* use the free method to initialize all important values to 0 */
  freeTransformData(&transform);

  transform0.userData = nullptr;
  transform0.vectors = nullptr;
  /* use the free method to initialize all important values to 0 */
  ...
  if (f != nullptr) {
    ...
    if ( (magic[0] == smileMagic[0]) &&
      ...
      if (i) {
        ...
        {
          size_t size_fread = 0;        
          size_fread = fread(&(magic[4-i]),i,1,f);
        }
      }
...
{
  if (tf != nullptr) {
    if (tf->userData != nullptr) { free(tf->userData); tf->userData = nullptr; }
    if (tf->vectors != nullptr)  { free(tf->vectors); tf->vectors = nullptr; }
    tf->head.nVec = 0;
    tf->head.nGroups = 0;
    tf->head.vecSize = 0;
    tf->head.nUserdata = 0;
...
{
  const char * endM = nullptr;
  if (turnEndMessage == nullptr) endM = "turnEnd";
  ...
  const char * startM = nullptr;
  if (turnStartMessage == nullptr) startM = "turnStart";
...
{
  if ((tf0 == nullptr)) { weight = 0.0; return; } /*||(tf0->head.nVec==0)*/
  if (tf != nullptr) {
    ...
    if ((tf0->userData!=nullptr)&&(tf->userData != nullptr)) {
      ...
      // zero-init
      if ((tf->userData != nullptr)&&(tf->head.nUserdata > 0)) {
        memset(tf->userData, 0, tf->head.nUserdata * sizeof(double));
      }
    }

    if ((tf0->vectors!=nullptr)&&(tf->vectors!=nullptr)&&(tf0->head.nVec > 0)&&(tf0->head.vecSize > 0)) {
      ...
      weight = 0.0;
      // zero-init:
      if ((tf->vectors != nullptr)&&(tf->head.nVec > 0)&&(tf->head.vecSize > 0)) {
...
/*      if (updateMethod == UPDMETHOD_AVGI) {
        if (bufferNframes == nullptr) bufferNframes = (long*)malloc(sizeof(long)*fixedBufferFrames);
      }*/
    }
  }
  ...
  // setup transform struct
    int init = 0;
    if ((transform.userData == nullptr)&&(transform.vectors==nullptr)) {
  ...
  if ((mode == MODE_ANALYSIS)||(mode == MODE_INCREMENTAL)) {
    ...
    if (mode == MODE_INCREMENTAL) {
      if ((Ndst <= 0) || (dst == nullptr)) return 0;
...
cVectorTransform::~cVectorTransform()
{
  ...
  if (bufferNframes!=nullptr) free(bufferNframes);
  freeTransformData(&transform);
  freeTransformData(&transform0);
}
