...
if (ct->setField("exWriter", "dataMemory writer for pcm voice excitation signal",
  ...
  */
  SMILECOMPONENT_IFNOTREGAGAIN(
    ...
    ct->setField("periodOutputFile", "Dump period start/end/amplitude/length to file if this option is set to a filename.", (const char*)nullptr);
...
cPitchJitter::cPitchJitter(const char *_name) :
  cDataProcessor(_name), minF0(25.0), // <- TODO: read this from pitch field metadata
    lastIdx(0), lastMis(0), out(nullptr), F0reader(nullptr), lastT0(0.0), lastDiff(0.0),
    ...
    filehandle(nullptr), threshCC_(0.5), minNumPeriods(2),
...
// save interpolated index of max peak in first frame in *maxI0 if != nullptr
// save interpolated index of max peak in second frame in *maxI1 if != nullptr
FLOAT_DMEM cPitchJitter::amplitudeDiff(FLOAT_DMEM *x, long Nx, FLOAT_DMEM *y, long Ny, double *maxI0, double *maxI1, FLOAT_DMEM *_A0, FLOAT_DMEM *_A1)
{
  ...
  double mi = smileMath_quadFrom3pts((double)(mI-1),x[mI-1],(double)(mI),x[mI],(double)(mI+1),x[mI+1],&A0,nullptr);
  //printf("A0 %f  x[%i] %f  Nx=%i\n",A0,mI,x[mI],Nx);
  if (maxI0 != nullptr) *maxI0 = mi;
  ...
  mi = smileMath_quadFrom3pts((double)(mI-1),y[mI-1],(double)(mI),y[mI],(double)(mI+1),y[mI+1],&A1,nullptr);
  if (maxI1 != nullptr) *maxI1 = mi;
  //printf("A1 %f  x[%i] %f  Ny=%i\n",A1,mI,y[mI],Ny);
  
  // save min to max amplitudes
  //if (_A0 != nullptr) *_A0 = (FLOAT_DMEM)A0;
  //if (_A1 != nullptr) *_A1 = (FLOAT_DMEM)A1;
  if (_A0 != nullptr) *_A0 = max0 - min0;
  if (_A1 != nullptr) *_A1 = max1 - min1;
...
// save interpolated index of max peak in first frame in *maxI0 if != nullptr  (rms amplitudes in A0, A1..! NOT peak)
// save interpolated index of max peak in second frame in *maxI1 if != nullptr
...
int cPitchJitter::myTick(long long t)
{
  ...
  if (fvec != nullptr) {
    ...
    if (maxRead < 1 || mat->dataF==nullptr) {
      SMILE_IERR(1,"maxRead < 1 or mat->dataF==nullptr, something is wrong (probably tmeta info on the pitch input level is not set correctly, thus the pitch frame length is read as 0; please debug the component that produces the pitch data this component reads!)");
      return 0;
    }
...
// TODO: send messages with pitch periods for pitch sync features!
    ...
    long n=0;
    if (out == nullptr) out = new cVector(Nout,DMEM_FLOAT);
...
////////////////////////////////////
    ...
    if (mat == nullptr) { 
      ...
      // TODO : write nullptr jitter/shimmer frame for proper sync?
      //...
      printf("Not avail\n");
      return 0; 
    }
