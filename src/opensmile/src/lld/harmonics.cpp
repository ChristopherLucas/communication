...
SMILECOMPONENT_REGCOMP(cHarmonics)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ...
    ct->setField("includeSingleElementFields", nullptr, 1);
    ...
    ct->setField("formantFrequencyFieldName", "Name of formant frequency field. Typcially formantFreqLpc", (const char*)nullptr);
    ...
    ct->setField("formantBandwidthFieldName", "Name of formant bandwidth field. Typically formantBandwidthLpc", (const char*)nullptr);
    ...
    ct->setField("harmonicDifferences", "Array that specifies harmonic differences (ratios in linear scale) to compute. Syntax for one element: H1-H2 (ratio of H1 to H2), H0 is fundamental frequency. A1,A2,...,AN is the amplitude (highest peak in the formant range) of the N-th formant, if formant frequency AND bandwidth information is given in the input (see formantFrequencyFieldName and formantBandwidthFieldName options).", (const char*)nullptr, ARRAY_TYPE);
...
cHarmonics::cHarmonics(const char *_name) :
  ...
  f0ElementName_(nullptr), magSpecFieldName_(nullptr), idxF0(-1), fsSec(-1.0),
  frq_(nullptr), nFrq_(0), harmonics_(nullptr),
  w_(nullptr), ip_(nullptr), acfdata_(nullptr), acf_(nullptr), cnt_(0),
  harmonicDifferences_(nullptr), haveFormantDifference_(false),
          ...
          *h2 = 0;
          ...
          if (h1[0] == 'H') {
            ...
            if ((r == 0) && (ep == h1 + 1)) {
              ...
              hd[i].h1formant = -1;
              hd[i].h1idx = r;
              if (maxHarmonic != nullptr && *maxHarmonic < r) {
                *maxHarmonic = r;
              }
            }
          } else if (h1[0] == 'A') {
            if (haveFormant != nullptr) {
              *haveFormant = true;
            }
            char *ep = nullptr;
            int r = strtol(h1 + 1, &ep, 10);
...
int cHarmonics::setupNewNames(long nEl)
{
  ...
  // find input fields
  if (f0ElementName_ == nullptr) {
    SMILE_IERR(1, "F0ElementName not specified! This is required! Aborting.");
    return 0;
  }
  if (magSpecFieldName_ == nullptr) {
    SMILE_IERR(1, "Magnitude spectrum field name not specified! This is required! Aborting.");
    return 0;
  }
  idxF0 = findElement(f0ElementName_, f0ElementNameIsFull_, nullptr, nullptr, nullptr);
  idxSpec = findField(magSpecFieldName_, magSpecFieldNameIsFull_, &nSpecBins, nullptr, -1, nullptr, &idxSpecField_);
  if (idxF0 == idxSpec) {
  ...
  idxFFreq_ = -1;
  idxFBandw_ = -1;
  if (formantFrequencyFieldName_ != nullptr) {
    if (formantBandwidthFieldName_ == nullptr) {
    ...
    idxFFreq_ = findField(formantFrequencyFieldName_, formantFrequencyFieldNameIsFull_, &nFFreq_, nullptr, -1, nullptr);
    idxFBandw_ = findField(formantBandwidthFieldName_, formantBandwidthFieldNameIsFull_, &nFBandw_, nullptr, -1, nullptr);
    // some sanity checks...
  ...
  if (nHarmonicDifferences_ > 0 && (harmonicDifferencesLog_ || harmonicDifferencesLinear_)) {
    for (int i = 0; i < nHarmonicDifferences_; i++) {
      if (harmonicDifferences_[i].text != nullptr) {
        ...
        SMILE_IERR(1, "Inconsistency in harmonic differences! Element %i has nullptr text. Seems like a bug somewhere. Aborting config.", i);
        return 0;
      }
    }
  }
    ...
    int nHarmonics, const double * frq, float F0)
{
  ...
  if (frq_ == nullptr) {  // linear fft spectrum
...
int cHarmonics::processVectorFloat(const FLOAT_DMEM *src, FLOAT_DMEM *dst, long Nsrc, long Ndst, int idxi) // idxi=input field index
{
  ...
  if (frq_ == nullptr) { // input block's frequency axis
    const FrameMetaInfo * fmeta = reader_->getFrameMetaInfo();
    if ((fmeta != nullptr)&&(idxSpecField_ < fmeta->N)) {
  ...
  //    }
      ...
      if (formantAmplitudes_) {
        for (int i = formantAmplitudesStart_; i <= formantAmplitudesEnd_; i++) {
          if (i > 0 && i <= nFFreq_ && faidxs != nullptr) {
            if (formantAmplitudesLinear_) {
              if (faidxs[i - 1] >= 0) {
...
cHarmonics::~cHarmonics()
{
  ...
  if (harmonicDifferences_ != nullptr) {
