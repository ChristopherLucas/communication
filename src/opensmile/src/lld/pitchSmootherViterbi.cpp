 ...
 */
FLOAT_DMEM cSmileViterbi::getNextOutputFrame(FLOAT_DMEM ** frame=nullptr, int *avail=nullptr, int *state=nullptr)
{
...
cPitchSmootherViterbi::cPitchSmootherViterbi(const char *_name) :
  cDataProcessor(_name), viterbi(nullptr), framePtr(nullptr), vecO(nullptr), buflen(0), outpVecSize(0),
...
int cPitchSmootherViterbi::setupNewNames(long nEl) 
{
  ...
  // totalCands = 0;
   ...
   for (i=0; i<nInputLevels; i++) {
     ...
     idx = findField("F0Cand",0,nCandidates+i,nullptr,-1,&more);
     f0candI[i] = idx;
...
{
  // cleanup...
  if (framePtr != nullptr) free(framePtr);
  if (viterbi != nullptr) delete viterbi;
  ...
  if (reader2 != nullptr) delete reader2;
}
