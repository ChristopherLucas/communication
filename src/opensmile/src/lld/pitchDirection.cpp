...
SMILECOMPONENT_REGCOMP(cPitchDirection)
{
  ...
  ct->setField("directionMsgRecp","Recipient component(s) for per syllable event-based pitch direction message (rise/fall/rise-fall/fall-rise message are sent only if and as ofter as a such event occurs on a syllable)",(const char *)nullptr);
...
cDataProcessor(_name), myVec(nullptr), F0field(-1), F0envField(0), 
stbuf(nullptr), ltbuf(nullptr), F0non0(0.0), lastF0non0(0.0),
...
turnStartMessage(nullptr), turnEndMessage(nullptr), invertTurn(0),
...
//                printf("Syl: Rise->Fall %i , %i\n",(int)((maxF0-startF0)/startF0*conf),(int)((maxF0-endF0)/startF0*conf));
  ...
  if (myVec == nullptr) myVec = new cVector(nEnabled,vec->type);
...
{
  // cleanup...
  if (myVec != nullptr) delete myVec;
  if (stbuf != nullptr) free(stbuf);
  if (ltbuf != nullptr) free(ltbuf);
}
