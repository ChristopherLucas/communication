...
SMILECOMPONENT_REGCOMP(cPitchShs)
{
  ...
  if (r2 == nullptr) {
    ...
    ct2->setField("dmLevel", nullptr, "___shs__dummy__");
  ...
  SMILECOMPONENT_IFNOTREGAGAIN( {} // <- this is only to avoid compiler warnings...
    ...
    ct->setField("inputFieldSearch",nullptr,"Mag_logScale");
    ...
    ct->setField("voicingCutoff",nullptr,0.70);
...
int cPitchShs::pitchDetect(FLOAT_DMEM * _inData, long _N, double _fsSec, double _baseT, FLOAT_DMEM *_f0cand, FLOAT_DMEM *_candVoice, FLOAT_DMEM *_candScore, long _nCandidates)
{
  ...
  for (i=0; i<nCand; i++) {
    ...
    double sc=0;
    double fx = smileMath_quadFrom3pts((double)f0, (double)SS[j-1], (double)f1, (double)SS[j], (double)f2, (double)SS[j+1], &sc, nullptr);
...
cPitchShs::~cPitchShs()
{
  if (SS != nullptr) free(SS);
  if (Fmap != nullptr) free(Fmap);
  if (shsVector_ != nullptr) delete(shsVector_);
  if (shsWriter_ != nullptr) delete(shsWriter_);
}
