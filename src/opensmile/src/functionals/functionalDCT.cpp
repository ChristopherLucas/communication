...
{
  if ((Nin>0)&&(Nout>0)) {
    if (costable != nullptr) free(costable);
    costable = (FLOAT_DMEM * )malloc(sizeof(FLOAT_DMEM)*Nin*Nout);
    costableNin = Nin;
    if (costable==nullptr) OUT_OF_MEMORY;
    int i,m;
...
long cFunctionalDCT::process(FLOAT_DMEM *in, FLOAT_DMEM *inSorted, FLOAT_DMEM *out, long Nin, long Nout)
{
  ...
  if ((Nin>0)&&(out!=nullptr)) {
    ...
    if (costable == nullptr || costableNin != Nin) {
      initCostable(Nin,Nout);
	    if (costable == nullptr) SMILE_IERR(1,"error initialising costable, probably Nin or Nout == 0 in cFunctionalDCT::process");
	  }
