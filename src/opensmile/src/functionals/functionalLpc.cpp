...
cFunctionalLpc::cFunctionalLpc(const char *_name) :
  ...
  firstCoeff(1),
  order(0), lpc(nullptr), acf(nullptr), tmpstr(nullptr)
{
}
...
void cFunctionalLpc::fetchConfig()
{
  ...
  if (acf == nullptr) acf = (FLOAT_DMEM *)malloc(sizeof(FLOAT_DMEM)*(order+1));
  if (lpc == nullptr) lpc = (FLOAT_DMEM *)malloc(sizeof(FLOAT_DMEM)*(order+1));
}
...
long cFunctionalLpc::process(FLOAT_DMEM *in, FLOAT_DMEM *inSorted, FLOAT_DMEM *out, long Nin, long Nout)
{
  ...
  if ((Nin>0)&&(out!=nullptr)) {
    ...
    FLOAT_DMEM gain = 0.0;
    FLOAT_DMEM *refl = nullptr; /* TODO: add support for reflection coefficients */
