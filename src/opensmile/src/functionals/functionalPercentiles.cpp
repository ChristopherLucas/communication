...
cFunctionalPercentiles::cFunctionalPercentiles(const char *_name) :
  ...
  pctl(nullptr),
  pctlr1(nullptr), pctlr2(nullptr),
  pctlq1(nullptr), pctlq2(nullptr),
  tmpstr(nullptr),
  quickAlgo(0),
  interp(0),
  varFctIdx(0)
...
//NO!! compute bin index from value!        if ((*in > cuint)&&(*in < cuint+unit)) { bins[i]++; }
      ...
      */
      ...
      if (inSorted == nullptr) {
        SMILE_ERR(1,"(inst '%s') expected sorted input, however got nullptr!",getInstName());
      }
      // quartiles:
      if (interp) {
...
{
  if(pctl!=nullptr) free(pctl);
  if(pctlr1!=nullptr) free(pctlr1);
  if(pctlr2!=nullptr) free(pctlr2);
  if(tmpstr!=nullptr) free(tmpstr);
}
