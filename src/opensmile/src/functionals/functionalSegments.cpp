...
cFunctionalSegments::cFunctionalSegments(const char *_name) :
  ...
  maxNumSeg(20), thresholds(nullptr), thresholdsTemp(nullptr), dbgPrint(0)
{
}
...
{
  if (thresholdsTemp != nullptr && thresholdsTemp != thresholds) free(thresholdsTemp);
  if (thresholds != nullptr) free(thresholds);
}
