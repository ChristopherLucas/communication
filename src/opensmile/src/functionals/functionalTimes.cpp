    ...
    FLOAT_DMEM max, FLOAT_DMEM mean, FLOAT_DMEM *out, long Nin, long Nout)
{
  if ((Nin>0)&&(out!=nullptr)) {
    ...
    if (useRobustPercentileRange_) {
      if (inSorted == nullptr) {
        SMILE_IERR(1, "Expected sorted input, however got nullptr! Fallback to max-min range instead of percentiles");
        range = max - min;
      } else {
...
{
  if(ultime!=nullptr) free(ultime);
  if(dltime!=nullptr) free(dltime);
}
