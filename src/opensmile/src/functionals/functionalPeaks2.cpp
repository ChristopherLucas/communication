...
SMILECOMPONENT_REGCOMP(cFunctionalPeaks2)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ...
    ct->setField("posDbgOutp","Filename for debug output of peak positions. The file will be created initially (unless 'posDbgAppend' is set to 1) and values for consecutive input frames will be appended, separated by a '---' marker line.",(const char *)nullptr);
...
cFunctionalPeaks2::cFunctionalPeaks2(const char *_name) :
  ...
  enabSlope(0), mmlistFirst(nullptr), mmlistLast(nullptr), dbg(nullptr), consoleDbg(0), useAbsThresh(0),
...
{
  if (listEl->prev != nullptr) {
    listEl->prev->next = listEl->next;
    if (listEl-> next != nullptr) {
...
long cFunctionalPeaks2::process(FLOAT_DMEM *in, FLOAT_DMEM *inSorted, FLOAT_DMEM min, FLOAT_DMEM max, FLOAT_DMEM mean, FLOAT_DMEM *out, long Nin, long Nout)
{
  long i;
  int n=0;
  if ((Nin>0)&&(out!=nullptr)) {
    if (posDbgOutp != nullptr && dbg == nullptr) {
      if (posDbgAppend) {
    ...
    struct peakMinMaxListEl * lastMaxPtr=nullptr;
    struct peakMinMaxListEl * lastMinPtr=nullptr;
    while (listEl != nullptr) { // iterate through list elements
      int doFree = 0;
