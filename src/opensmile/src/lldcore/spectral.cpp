...
SMILECOMPONENT_REGCOMP(cSpectral)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ...
    ct->setField("slopes","slopes[n] = LoFrq[Hz]-HiFrq[Hz] (e.g. 0-5000), computes spectral slope in the given band (only integer frequencies are allowed!)", (const char*)nullptr, ARRAY_TYPE);
...
cSpectral::cSpectral(const char *_name) :
  ...
  bandsL(nullptr), bandsH(nullptr), 
  slopesL(nullptr), slopesH(nullptr),
  rollOff(nullptr),
  bandsLi(nullptr), bandsHi(nullptr),
  wghtLi(nullptr), wghtHi(nullptr),
  slopeBandsLi(nullptr), slopeBandsHi(nullptr),
  slopeWghtLi(nullptr), slopeWghtHi(nullptr),
  fsSec(-1.0), frq(nullptr), frqScale(-1), frqScaleParam(0.0),
  ...
  prevSpec(nullptr), nSrcPrevSpec(nullptr), nFieldsPrevSpec(0)
{
...
{
  if (text == nullptr || lowerHz == nullptr || upperHz == nullptr) {
    return;
  }
...
int cSpectral::parseBandsConfig(const char * field, long ** bLow, long ** bHigh)
{
  ...
  if (N > 0) {
    ...
    if (bLow != nullptr) {
      if (*bLow != nullptr) free(bLow);
      *bLow = bL;
    }
    if (bHigh != nullptr) {
      if (*bHigh != nullptr) free(bHigh);
      *bHigh = bH;
    }
  }
  return N;
...
int cSpectral::processVectorFloat(const FLOAT_DMEM *src, FLOAT_DMEM *dst, long Nsrc, long Ndst, int idxi) // idxi=input field index
{
  ...
  if (frq == nullptr) { // input block's frequency axis
    // TODO: use frqSet flag to avoid doing the code below on every tick if info==nullptr
  ...
  if (frqScale == -1) {
      ...
      if (mdata != nullptr && mdata->ID == 1001 /* SCALED_SPEC */) {
  ...
  const FLOAT_DMEM *srcM = nullptr; // magnitude spectrum
  const FLOAT_DMEM *srcL = nullptr; // log spectrum
  const FLOAT_DMEM *srcP = nullptr; // power spectrum
  const FLOAT_DMEM *srcLP = nullptr; // log or power spectrum
  ...
  if (requireLogSpec) {
    ...
    const FLOAT_DMEM *mySrc = nullptr;
    if (requirePowerSpec) {
  ...
  */
  ...
  for (i=0; i<nBands; i++) {
    if (isBandValid(bandsL[i],bandsH[i])) {
      double idxL;
      double wghtL;
      if ((nScale < Nsrc)||(frq==nullptr)) {
        idxL =  (double)bandsL[i] / F0 ;
        wghtL =  ceil(idxL) - idxL;
      } else {
        if (bandsLi == nullptr) { // map the frequency to an fft bin index
          int ii,iii;
      ...
      if ((nScale < Nsrc)||(frq==nullptr)) {
        ...
        if (bandsHi == nullptr) { // map the frequency to an fft bin index
          int ii, iii;
  ...
  for (i=0; i<nSlopes; i++) {
    if (isBandValid(slopesL[i], slopesH[i])) {
      ...
      if ((nScale < Nsrc) || (frq == nullptr)) {
        ...
        if (slopeBandsLi == nullptr) { // map the frequency to an fft bin index
          int ii, iii;
      ...
      if ((nScale < Nsrc) || (frq == nullptr)) {
        ...
        if (slopeBandsHi == nullptr) { // map the frequency to an fft bin index
          int ii, iii;
      ...
      if ((nScale >= Nsrc && nScale > 0) && (frq != nullptr)) {
  ...
  if (specPosDiff || specDiff || flux || fluxCentroid || fluxAtFluxCentroid) {
    ...
    if (prevSpec[idxi] == nullptr) {
      ...
      if (fluxCentroid) {
        if ((nScale>specRangeUpperBin)&&(frq!=nullptr)) {
          for (j = specRangeLowerBin; j <= specRangeUpperBin; j++) {
            double myB = ((double)srcM[j]/normC - (double)magP[j - specRangeLowerBin]/normP);
            myAf += myB*myB * frq[j];
          }
  ...
  if (sharpness) {
   ...
   if (srcP != nullptr && srcP != src) {
     free((FLOAT_DMEM*)srcP);
   }
   if (srcM != nullptr && srcM != src) {
     free((FLOAT_DMEM*)srcM);
   }
   if (srcL != nullptr && srcL != src) {
...
cSpectral::~cSpectral()
{
  ...
  if (bandsL!=nullptr) free(bandsL);
  if (bandsH!=nullptr) free(bandsH);
  if (slopesL!=nullptr) free(slopesL);
  if (slopesH!=nullptr) free(slopesH);
  if (bandsLi!=nullptr) free(bandsLi);
  if (bandsHi!=nullptr) free(bandsHi);
  if (wghtLi!=nullptr) free(wghtLi);
  if (wghtHi!=nullptr) free(wghtHi);
  if (slopeBandsLi!=nullptr) free(slopeBandsLi);
  if (slopeBandsHi!=nullptr) free(slopeBandsHi);
  if (slopeWghtLi!=nullptr) free(slopeWghtLi);
  if (slopeWghtHi!=nullptr) free(slopeWghtHi);
  if (rollOff!=nullptr) free(rollOff);
  if (prevSpec != nullptr) {
    for (int i = 0; i < nFieldsPrevSpec; i++) {
      if (prevSpec[i] != nullptr) {
        free(prevSpec[i]);
      }
    }
    free(prevSpec);
