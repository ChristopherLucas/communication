...
SMILECOMPONENT_REGCOMP(cPlp)
{
	...
	SMILECOMPONENT_IFNOTREGAGAIN(
			//ct->setField("nameAppend", nullptr, ");
...
htkcompatible(0),
rasta_buf_iir(nullptr),
rasta_buf_fir(nullptr),
acf(nullptr), lpc(nullptr), ceps(nullptr),
eqlCurve(nullptr),
costable(nullptr),
sintable(nullptr)
{
...
int cPlp::dataProcessorCustomFinalise()
{
	...
	if (eqlCurve == nullptr) eqlCurve = (FLOAT_DMEM**)multiConfAlloc();
	if (acf == nullptr) acf = (FLOAT_DMEM**)multiConfAlloc();
	if (lpc == nullptr) lpc = (FLOAT_DMEM**)multiConfAlloc();
	if (ceps == nullptr) ceps = (FLOAT_DMEM**)multiConfAlloc();
	if (rasta_buf_fir == nullptr) rasta_buf_fir = (FLOAT_DMEM**)multiConfAlloc();
	if (rasta_buf_iir == nullptr) rasta_buf_iir = (FLOAT_DMEM**)multiConfAlloc();
...
//		rasta_buf_fir_ptr = (rasta_buf_fir_ptr+1)%5;
	...
	if (doIDFT) {  // TODO: check nAuto<=nDst!
		...
		if (doLP) {
			smileDsp_calcLpcAcf(_acf, _lpc, lpOrder, &lpGain, nullptr);
