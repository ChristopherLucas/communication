...
cPitchBase::cPitchBase(const char *_name) :
  ...
  inData(nullptr), f0cand(nullptr), candVoice(nullptr), candScore(nullptr)
{
...
//  fsSec = getFrameSizeSec();
  ...
  if (candScore == nullptr) candScore = (FLOAT_DMEM*)calloc(1,sizeof(FLOAT_DMEM)*nCandidates);
  if (candVoice == nullptr) candVoice = (FLOAT_DMEM*)calloc(1,sizeof(FLOAT_DMEM)*nCandidates);
  if (f0cand == nullptr) f0cand = (FLOAT_DMEM*)calloc(1,sizeof(FLOAT_DMEM)*nCandidates);
...
int cPitchBase::processVectorFloat(const FLOAT_DMEM *src, FLOAT_DMEM *dst, long Nsrc, long Ndst, int idxi) // idxi=input field index
{
 ...
 // for (i=0; i<Nsrc; i++) { if (src[i] != 9.0) Rprintf("src[%i]\n",i); }
 // for (i=0; i<nInput; i++) { if (inData[i] != 9.0) Rprintf("inData[%i]\n",i); }
