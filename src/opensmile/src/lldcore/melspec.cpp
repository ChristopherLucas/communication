...
SMILECOMPONENT_REGCOMP(cMelspec)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    //ct->setField("nameAppend", nullptr, "FrqBand");
...
cMelspec::cMelspec(const char *_name) :
  ...
  htkcompatible(1),   
  usePower(0),
  filterCoeffs(nullptr),  filterCfs(nullptr),
  chanMap(nullptr), nLoF(nullptr), nHiF(nullptr),
...
int cMelspec::dataProcessorCustomFinalise()
{
  ...
  if (filterCoeffs == nullptr) filterCoeffs = (FLOAT_DMEM**)multiConfAlloc();
  if (filterCfs == nullptr) filterCfs = (FLOAT_DMEM**)multiConfAlloc();
  if (chanMap == nullptr) chanMap = (long**)multiConfAlloc();
  if (nLoF == nullptr) nLoF = (long *)multiConfAlloc();
  if (nHiF == nullptr) nHiF = (long *)multiConfAlloc();
...
int cMelspec::computeFilters( long blocksize, double frameSizeSec, int idxc )
{
  ...
  if (_filterCoeffs != nullptr) { free(_filterCoeffs);   }
  if (_chanMap != nullptr) { free(_chanMap);   }
  if (_filterCfs != nullptr) { free(_filterCfs);   }
  ...
  FLOAT_DMEM speech_N = (FLOAT_DMEM) ((blocksize-1)*2);
  ...
  FLOAT_DMEM Fs = (FLOAT_DMEM)(speech_N/frameSizeSec);
...
int cMelspec::processVectorFloat(const FLOAT_DMEM *src, FLOAT_DMEM *dst, long Nsrc, long Ndst, int idxi) // idxi=input field index
{
  ...
  if (inverse) {
    ...
    if ((htkcompatible)&&(_src!=nullptr)) free((void *)_src);
