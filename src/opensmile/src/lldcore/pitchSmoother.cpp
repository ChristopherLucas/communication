...
cPitchSmoother::cPitchSmoother(const char *_name) :
  ...
  nCands(nullptr), nCandidates(nullptr), 
  f0cand(nullptr), candVoice(nullptr), candScore(nullptr),
  voicingCutoff(nullptr),
  f0candI(nullptr), candVoiceI(nullptr), candScoreI(nullptr),
  voicingC1I(nullptr), F0rawI(nullptr), voicingClipI(nullptr),
  median0WorkspaceF0cand(nullptr), lastFinal(nullptr),
...
void cPitchSmoother::fetchConfig()
{
  ...
  if (postSmoothingMethodStr != nullptr){
...
int cPitchSmoother::setupNewNames(long nEl)
{
  ...
  for (i=0; i<nInputLevels; i++) {
    ...
    if (F0raw) {
      int idxRa = findField("F0raw",0,nullptr,nullptr,-1,&moreRa); // TODO: check if nC == 1 here
      //printf("idxRa %i moreRa %i  i %i\n",idxRa,moreRa,i);
      F0rawI[i] = idxRa;
      if (moreRa>0) { moreRa = idxRa+1; }
    }
    if (voicingClip) {
      int idxVc = findField("voicingClip",0,nullptr,nullptr,-1,&moreVc); // TODO: check if nC == 1 here
      voicingClipI[i] = idxVc;
      if (moreVc>0) { moreVc = idxVc+1; }
    }
    if (voicingC1) {
      int idxC1 = findField("voicingC1",0,nullptr,nullptr,-1,&moreC1); // TODO: check if nC == 1 here
      voicingC1I[i] = idxC1;
      if (moreC1>0) { moreC1 = idxC1+1; }
    }

    idx = findField("F0Cand",0,&nC,nullptr,-1,&more);
    f0candI[i] = idx;
    //printf("idx %i\n",idx);
    if (idx >= 0) {
      int idxV = findField("candVoicing",0,nullptr,nullptr,-1,&moreV); 
      candVoiceI[i] = idxV;
      if (moreV>0) { moreV = idxV+1; }
      int idxS = findField("candScore",0,nullptr,nullptr,-1,&moreS); // TODO: check if nC == N here
      candScoreI[i] = idxS;
  ...
  if (f0cand == nullptr) f0cand = (FLOAT_DMEM*)calloc(1,sizeof(FLOAT_DMEM)*totalCands*3);
  ...
  if (F0final||F0finalEnv) {
    ...
    if (postSmoothing) {
      if (postSmoothingMethod == POSTSMOOTHING_SIMPLE) {
        ...
        lastFinal[0] = pitch;
        pitchOut /*dst[n]*/ = smileMath_median( lastFinal, postSmoothing, nullptr );
...
{
  if (voicingCutoff != nullptr) free(voicingCutoff);
  if (nCandidates != nullptr) free(nCandidates);
  if (nCands != nullptr) free(nCands);
  if (f0cand != nullptr) free(f0cand);
  //if (candVoice != nullptr) free(candVoice);
  //if (candScore != nullptr) free(candScore);
  if (f0candI != nullptr) free(f0candI);
  if (candVoiceI != nullptr) free(candVoiceI);
  if (candScoreI != nullptr) free(candScoreI);
  if (F0rawI != nullptr) free(F0rawI);
  if (voicingClipI != nullptr) free(voicingClipI);
  if (voicingC1I != nullptr) free(voicingC1I);
  if (median0WorkspaceF0cand != nullptr) smileUtil_temporalMedianFilterFree(median0WorkspaceF0cand);
  if (lastFinal != nullptr) free(lastFinal);
}
